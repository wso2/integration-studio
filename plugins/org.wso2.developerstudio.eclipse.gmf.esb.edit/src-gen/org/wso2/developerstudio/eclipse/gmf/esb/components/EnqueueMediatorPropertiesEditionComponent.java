...
public class EnqueueMediatorPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
     ...
     * 
     */
    public EnqueueMediatorPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject enqueueMediator,
            String editing_mode) {
        ...
        if (editingPart != null && key == partKey) {
            ...
            if (isAccessible(EsbViewsRepository.EnqueueMediator.Properties.description))
                basePart.setDescription(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING,
                        enqueueMediator.getDescription()));
            ...
            if (isAccessible(EsbViewsRepository.EnqueueMediator.Properties.executor))
                basePart.setExecutor(
                        EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, enqueueMediator.getExecutor()));

            if (isAccessible(EsbViewsRepository.EnqueueMediator.Properties.priority)) {
                basePart.setPriority(
                        EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, enqueueMediator.getPriority()));
            }
    ...
    public void updateSemanticModel(final IPropertiesEditionEvent event) {
        ...
        if (EsbViewsRepository.EnqueueMediator.Properties.description == event.getAffectedEditor()) {
            enqueueMediator.setDescription((java.lang.String) EEFConverterUtil
        ...
        if (EsbViewsRepository.EnqueueMediator.Properties.executor == event.getAffectedEditor()) {
            enqueueMediator.setExecutor((java.lang.String) EEFConverterUtil
                    .createFromString(EcorePackage.Literals.ESTRING, (String) event.getNewValue()));
        }
        if (EsbViewsRepository.EnqueueMediator.Properties.priority == event.getAffectedEditor()) {
            enqueueMediator.setPriority(
    ...
    public void updatePart(Notification msg) {
        ...
        if (editingPart.isVisible()) {
            ...
            if (EsbPackage.eINSTANCE.getEsbElement_Description().equals(msg.getFeature())
                    ...
                    && isAccessible(EsbViewsRepository.EnqueueMediator.Properties.description)) {
            ...
            if (EsbPackage.eINSTANCE.getEsbElement_CommentsList().equals(msg.getFeature())
                    && msg.getNotifier().equals(semanticObject) && basePart != null
                    && isAccessible(EsbViewsRepository.EnqueueMediator.Properties.commentsList)) {
            ...
            if (EsbPackage.eINSTANCE.getMediator_Reverse().equals(msg.getFeature())
                    && msg.getNotifier().equals(semanticObject) && basePart != null
                    && isAccessible(EsbViewsRepository.EnqueueMediator.Properties.reverse))
                basePart.setReverse((Boolean) msg.getNewValue());

            if (EsbPackage.eINSTANCE.getEnqueueMediator_Executor().equals(msg.getFeature())
                    && msg.getNotifier().equals(semanticObject) && basePart != null
                    && isAccessible(EsbViewsRepository.EnqueueMediator.Properties.executor)) {
                if (msg.getNewValue() != null) {
            ...
            if (EsbPackage.eINSTANCE.getEnqueueMediator_Priority().equals(msg.getFeature())
                    && msg.getNotifier().equals(semanticObject) && basePart != null
                    && isAccessible(EsbViewsRepository.EnqueueMediator.Properties.priority)) {
                if (msg.getNewValue() != null) {
    ...
    protected NotificationFilter[] getNotificationFilters() {
        NotificationFilter filter = new EStructuralFeatureNotificationFilter(
                ...
                EsbPackage.eINSTANCE.getMediator_Reverse(), EsbPackage.eINSTANCE.getEnqueueMediator_Executor(),
        ...
        if (event.getNewValue() != null) {
            try {
                ...
                if (EsbViewsRepository.EnqueueMediator.Properties.executor == event.getAffectedEditor()) {
                    ...
                    if (newValue instanceof String) {
                        newValue = EEFConverterUtil.createFromString(
                                EsbPackage.eINSTANCE.getEnqueueMediator_Executor().getEAttributeType(),
                                (String) newValue);
                    }
                    ret = Diagnostician.INSTANCE
                            .validate(EsbPackage.eINSTANCE.getEnqueueMediator_Executor().getEAttributeType(), newValue);
                }
                if (EsbViewsRepository.EnqueueMediator.Properties.priority == event.getAffectedEditor()) {
                    Object newValue = event.getNewValue();
                    if (newValue instanceof String) {
                        newValue = EEFConverterUtil.createFromString(
                                EsbPackage.eINSTANCE.getEnqueueMediator_Priority().getEAttributeType(),
                                (String) newValue);
                    }
                    ret = Diagnostician.INSTANCE
                            .validate(EsbPackage.eINSTANCE.getEnqueueMediator_Priority().getEAttributeType(), newValue);
                }
