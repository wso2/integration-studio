...
public class ESBEditorPasteCommand extends org.eclipse.gmf.runtime.common.core.command.AbstractCommand {
    ...
    private void duplicate() {
        ...
        if (part instanceof IDiagramWorkbenchPart) {
            Command cmd = ((IDiagramWorkbenchPart) part).getDiagramEditPart().getCommand((Request) request);
    ...
    private void storeAndDelete(List<EsbLinkEditPart> connections, IGraphicalEditPart editPart) {
        if (!connections.isEmpty()) {
            if (connections.size() > 1) {
                /*
                 * There was a concurrent exception due to 'for-each' loop. So that we have put a 'if-else' for aviod
                 * disturbance for elements having
                 * only one link. But now we have get rid of 'for-each' and have put old 'for' loop. But didn't remove
                 * if-else. We can remove 'if-else' after doing
                 * adequate testing.
                 */
    ...
    private void deleteLink(EsbLinkEditPart link, IGraphicalEditPart editPart) {
        ...
        col.add(((org.eclipse.gmf.runtime.notation.impl.ConnectorImpl) link.getModel()).getElement());
        ...
        DeleteCommand delCmd = new DeleteCommand(link.getNotationView());
    ...
    private void connect() {
        for (int i = 0; i < connectors.keySet().toArray().length; ++i) {
            ConnectionUtils.createConnection((AbstractConnectorEditPart) connectors.keySet().toArray()[i],
                    connectors.get(connectors.keySet().toArray()[i]));
        }
    }
