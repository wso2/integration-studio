...
public class SmooksMediatorDeserializer extends AbstractEsbNodeDeserializer<AbstractMediator, SmooksMediator> {
    ...
    public SmooksMediator createNode(IGraphicalEditPart part, AbstractMediator mediator) {
        Assert.isTrue(mediator instanceof org.wso2.carbon.mediator.transform.SmooksMediator,
        ...
        SmooksMediator visualSmooksMediator = (SmooksMediator) DeserializerUtils.createNode(part,
        ...
        if (smooksMediator.getInput().getExpression() != null) {
            executeSetValueCommand(SMOOKS_MEDIATOR__INPUT_EXPRESSION,
                    createNamespacedProperty(smooksMediator.getInput().getExpression()));
        }
        ...
        if (smooksMediator.getOutput().getExpression() != null) {
            executeSetValueCommand(SMOOKS_MEDIATOR__OUTPUT_EXPRESSION,
                    createNamespacedProperty(smooksMediator.getOutput().getExpression()));
        ...
        executeSetValueCommand(visualSmooksMediator.getConfigurationKey(), REGISTRY_KEY_PROPERTY__KEY_VALUE,
                smooksMediator.getConfigKey());
