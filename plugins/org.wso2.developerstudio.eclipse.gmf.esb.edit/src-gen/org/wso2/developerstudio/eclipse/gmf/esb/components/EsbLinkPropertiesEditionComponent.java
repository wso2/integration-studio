...
public class EsbLinkPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
     ...
     * 
     */
    public EsbLinkPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject esbLink,
            String editing_mode) {
                ...
                basePart.addFilterToSource(new ViewerFilter() {
                    ...
                    public boolean select(Viewer viewer, Object parentElement, Object element) {
                        return (element instanceof String && element.equals("")) //$NON-NLS-1$
                                || (element instanceof OutputConnector);
                    }
    ...
    public void updateSemanticModel(final IPropertiesEditionEvent event) {
        ...
        if (EsbViewsRepository.EsbLink.Properties.source == event.getAffectedEditor()) {
            if (event.getKind() == PropertiesEditionEvent.SET) {
                ...
                EReferencePropertiesEditionContext context = new EReferencePropertiesEditionContext(editingContext,
                        this, sourceSettings, editingContext.getAdapterFactory());
        ...
        if (EsbViewsRepository.EsbLink.Properties.target == event.getAffectedEditor()) {
            if (event.getKind() == PropertiesEditionEvent.SET) {
                ...
                EReferencePropertiesEditionContext context = new EReferencePropertiesEditionContext(editingContext,
                        this, targetSettings, editingContext.getAdapterFactory());
                PropertiesEditingProvider provider = (PropertiesEditingProvider) editingContext.getAdapterFactory()
                        .adapt(semanticObject, PropertiesEditingProvider.class);
                if (provider != null) {
                    PropertiesEditingPolicy policy = provider.getPolicy(context);
    ...
    public void updatePart(Notification msg) {
        ...
        if (editingPart.isVisible()) {
            ...
            if (EsbPackage.eINSTANCE.getEsbLink_Source().equals(msg.getFeature()) && basePart != null
                    && isAccessible(EsbViewsRepository.EsbLink.Properties.source))
                basePart.setSource((EObject) msg.getNewValue());
            if (EsbPackage.eINSTANCE.getEsbLink_Target().equals(msg.getFeature()) && basePart != null
                    && isAccessible(EsbViewsRepository.EsbLink.Properties.target))
    ...
    protected NotificationFilter[] getNotificationFilters() {
        NotificationFilter filter = new EStructuralFeatureNotificationFilter(EsbPackage.eINSTANCE.getEsbLink_Source(),
