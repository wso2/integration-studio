...
public class ConfigureConditionalRouterMediatorDialog extends Dialog {
     ...
     * @param parentShell
     */
    public ConfigureConditionalRouterMediatorDialog(Shell parentShell, ConditionalRouterMediator router,
        ...
        cmdSetEvaluatorExpression.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                if (tblRoutes.getSelectionIndex() != -1) {
                    ...
                    EvaluatorExpressionEditorDialog dialog = new EvaluatorExpressionEditorDialog(getShell(),
                            evaluatorExpression);
                    dialog.create();
                    ...
                    if (dialog.getReturnCode() == Window.OK) {
                        ...
                        item.setText("Route [" + evaluatorExpression.getEvaluatorValue()
            ...
            public void widgetSelected(SelectionEvent e) {
                if (tblRoutes.getSelectionIndex() != -1) {
                    ...
                    RegistryKeyProperty sequenceKey = EsbFactory.eINSTANCE
                            .copyRegistryKeyProperty(wrapper.getTargetSequence());
                    RegistryKeyPropertyEditorDialog dialog = new RegistryKeyPropertyEditorDialog(getShell(), SWT.TITLE,
                            sequenceKey, RegistryKeyPropertyUtil.findLocalNamedEntities(wrapper.getConditionalRoute()));
                    dialog.create();
    ...
    private TableItem bindRoute(RouteWrapper wrapper) {
        ...
        item.setText("Route");
        item.setImage(SWTResourceManager.getImage(this.getClass(), "/icons/custom/12px/conditional-router.png"));
        ...
        wrapper.setEvaluatorExpression(
                EsbFactory.eINSTANCE.copyEvaluatorExpressionProperty(conditionalRoute.getEvaluatorExpression()));
        wrapper.setTargetSequence(EsbFactory.eINSTANCE.copyRegistryKeyProperty(conditionalRoute.getTargetSequence()));
        item.setText("Route [" + wrapper.getEvaluatorExpression().getEvaluatorValue()
                .replaceAll(System.getProperty("line.separator", "\n"), "") + "]");
    ...
    private void unbindRoute(int itemIndex) {
        ...
        if (null != conditionalRoute.eContainer()) {
            RemoveCommand removeCmd = new RemoveCommand(editingDomain, conditionalRouter,
                    EsbPackage.Literals.CONDITIONAL_ROUTER_MEDIATOR__CONDITIONAL_ROUTE_BRANCHES, conditionalRoute);
    ...
    protected void okPressed() {
        for (TableItem item : tblRoutes.getItems()) {
            ...
            if (null == conditionalRoute.eContainer()) {
                AddCommand addCmd = new AddCommand(editingDomain, conditionalRouter,
                        EsbPackage.Literals.CONDITIONAL_ROUTER_MEDIATOR__CONDITIONAL_ROUTE_BRANCHES, conditionalRoute);
            ...
            if (!conditionalRoute.getEvaluatorExpression().equals(wrapper.getEvaluatorExpression())) {
                setCmd = new SetCommand(editingDomain, conditionalRoute,
            ...
            if (conditionalRoute.isBreakAfterRoute() != wrapper.isBreakAfterRoute()) {
                setCmd = new SetCommand(editingDomain, conditionalRoute,
                        EsbPackage.Literals.CONDITIONAL_ROUTE_BRANCH__BREAK_AFTER_ROUTE, wrapper.isBreakAfterRoute());
                getResultCommand().append(setCmd);
            }

            if (!conditionalRoute.getTargetSequence().equals(wrapper.getTargetSequence())) {
                setCmd = new SetCommand(editingDomain, conditionalRoute,
                        EsbPackage.Literals.CONDITIONAL_ROUTE_BRANCH__TARGET_SEQUENCE, wrapper.getTargetSequence());
                getResultCommand().append(setCmd);
            }
