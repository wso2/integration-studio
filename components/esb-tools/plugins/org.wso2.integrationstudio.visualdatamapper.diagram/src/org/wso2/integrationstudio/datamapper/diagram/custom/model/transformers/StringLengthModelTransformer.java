...
import org.wso2.integrationstudio.datamapper.impl.StringLengthImpl;

public class StringLengthModelTransformer implements IModelTransformer {

	@Override
	public DMOperation transform(OperatorImpl operator, int index) {
		if (operator instanceof StringLengthImpl) {
			DMOperation stringLengthOperatorModel = new DMOperation(DataMapperOperatorType.STRING_LENGTH, operator.toString(), index);
			return stringLengthOperatorModel;
		}
		throw new IllegalArgumentException("StringLengthImpl operator expected. Found : " + operator.toString());
	}
