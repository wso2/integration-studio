...
package org.wso2.developerstudio.eclipse.artifact.cep.receiver.validators;
...
public class ReceiverFieldController extends AbstractFieldController {
	...
	public void validate(String modelProperty, Object value, ProjectDataModel model)
		...
		if (modelProperty.equals("project.name")) {
			...
			if (value != null) {
				...
				ReceiverModel esModel = (ReceiverModel) model;
				if (esModel != null) {
					IContainer resLocation = esModel.getReceiverSaveLocation();
					if (resLocation != null) {
	...
	public boolean isVisibleField(String modelProperty, ProjectDataModel model) {
		...
		if (modelProperty.equals("available.eventstream")) {
			List<OMElement> availableEventstreamList = ((ReceiverModel) model).getAvailableEventstreamList();
