...
public class ValidateFeatureDialog extends Dialog {
    ...
    public ValidateFeatureDialog(Shell parentShell, ValidateMediator validateMediator) {
        super(parentShell);
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        featureTable = new Table(container, SWT.BORDER | SWT.FULL_SELECTION | SWT.HIDE_SELECTION);
        ...
        addBtn.addListener(SWT.Selection, new Listener() {
            ...
            public void handleEvent(Event event) {
                ...
                TableItem item = bindFeature(EsbFactory.eINSTANCE.createValidateFeature());
    ...
    private TableItem bindFeature(ValidateFeature feature) {
        ...
        item.setText(new String[] { feature.getFeatureName(), Boolean.toString(feature.isFeatureEnabled()) });
        ...
        for (TableItem item : featureTable.getItems()) {
            ...
            if (feature.eContainer() == null) {
                ...
                if (!item.getText(0).equals(feature.getFeatureName())) {
                    SetCommand setTypeCmd = new SetCommand(editingDomain, feature,
                            EsbPackage.Literals.ABSTRACT_BOOLEAN_FEATURE__FEATURE_NAME, item.getText(0));
                    getResultCommand().append(setTypeCmd);
                }

                if (!item.getText(1).equals(Boolean.toString(feature.isFeatureEnabled()))) {

                    SetCommand setTypeCmd = new SetCommand(editingDomain, feature,
                            EsbPackage.Literals.ABSTRACT_BOOLEAN_FEATURE__FEATURE_ENABLED, item.getText(1));
                    getResultCommand().append(setTypeCmd);
                }
            }
        }
