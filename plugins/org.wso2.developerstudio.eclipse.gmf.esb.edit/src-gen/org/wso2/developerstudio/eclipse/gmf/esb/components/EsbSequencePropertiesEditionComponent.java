...
public class EsbSequencePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
     ...
     * 
     */
    public EsbSequencePropertiesEditionComponent(PropertiesEditingContext editingContext, EObject esbSequence,
            String editing_mode) {
    ...
    public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
        ...
        if (editingPart != null && key == partKey) {
            ...
            if (isAccessible(EsbViewsRepository.EsbSequence.Properties.name))
                basePart.setName(
                        EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, esbSequence.getName()));

            if (isAccessible(EsbViewsRepository.EsbSequence.Properties.childMediators)) {
                childMediatorsSettings = new ReferencesTableSettings(esbSequence,
    ...
    public void updateSemanticModel(final IPropertiesEditionEvent event) {
        ...
        if (EsbViewsRepository.EsbSequence.Properties.name == event.getAffectedEditor()) {
            esbSequence.setName((java.lang.String) EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING,
        ...
        if (EsbViewsRepository.EsbSequence.Properties.childMediators == event.getAffectedEditor()) {
            if (event.getKind() == PropertiesEditionEvent.ADD) {
                EReferencePropertiesEditionContext context = new EReferencePropertiesEditionContext(editingContext,
                        this, childMediatorsSettings, editingContext.getAdapterFactory());
    ...
    public void updatePart(Notification msg) {
        ...
        if (editingPart.isVisible()) {
            ...
            if (EsbPackage.eINSTANCE.getEsbSequence_Name().equals(msg.getFeature())
                    && msg.getNotifier().equals(semanticObject) && basePart != null
                    && isAccessible(EsbViewsRepository.EsbSequence.Properties.name)) {
            ...
            if (EsbPackage.eINSTANCE.getEsbSequence_ChildMediators().equals(msg.getFeature())
                    && isAccessible(EsbViewsRepository.EsbSequence.Properties.childMediators))
    ...
    protected NotificationFilter[] getNotificationFilters() {
        NotificationFilter filter = new EStructuralFeatureNotificationFilter(EsbPackage.eINSTANCE.getEsbSequence_Name(),
                EsbPackage.eINSTANCE.getEsbSequence_ChildMediators());
                ...
                if (EsbViewsRepository.EsbSequence.Properties.name == event.getAffectedEditor()) {
                    ...
                    if (newValue instanceof String) {
                        newValue = EEFConverterUtil.createFromString(
                                EsbPackage.eINSTANCE.getEsbSequence_Name().getEAttributeType(), (String) newValue);
                    }
                    ret = Diagnostician.INSTANCE
                            .validate(EsbPackage.eINSTANCE.getEsbSequence_Name().getEAttributeType(), newValue);
                }
