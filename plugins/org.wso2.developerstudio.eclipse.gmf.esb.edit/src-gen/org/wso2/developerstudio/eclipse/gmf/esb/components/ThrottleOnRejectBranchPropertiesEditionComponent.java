...
public class ThrottleOnRejectBranchPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
     ...
     * 
     */
    public ThrottleOnRejectBranchPropertiesEditionComponent(PropertiesEditingContext editingContext,
            EObject throttleOnRejectBranch, String editing_mode) {
        ...
        if (editingPart != null && key == partKey) {
            ...
            if (isAccessible(EsbViewsRepository.ThrottleOnRejectBranch.Properties.sequenceType)) {
                basePart.initSequenceType(
                        EEFUtils.choiceOfValues(throttleOnRejectBranch,
                                EsbPackage.eINSTANCE.getThrottleOnRejectBranch_SequenceType()),
                        throttleOnRejectBranch.getSequenceType());
            }
            // init filters
    ...
    public void updatePart(Notification msg) {
        ...
        if (editingPart.isVisible()) {
            ...
            if (EsbPackage.eINSTANCE.getThrottleOnRejectBranch_SequenceType().equals(msg.getFeature())
                    && msg.getNotifier().equals(semanticObject)
                    && isAccessible(EsbViewsRepository.ThrottleOnRejectBranch.Properties.sequenceType))
                ...
                if (EsbViewsRepository.ThrottleOnRejectBranch.Properties.sequenceType == event.getAffectedEditor()) {
                    ...
                    if (newValue instanceof String) {
                        newValue = EEFConverterUtil.createFromString(
                                EsbPackage.eINSTANCE.getThrottleOnRejectBranch_SequenceType().getEAttributeType(),
                                (String) newValue);
                    }
