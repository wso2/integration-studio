...
public class ConfigureXQueryMediatorDialog extends Dialog {
    ...
    public ConfigureXQueryMediatorDialog(Shell parentShell, XQueryMediator xqueryMediator,
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        newXQueryVariableButton = new Button(container, SWT.NONE);
        {
            ...
            FormData newXQueryVariableButtonLayoutData = new FormData(80, SWT.DEFAULT);
            newXQueryVariableButtonLayoutData.top = new FormAttachment(xqueryVariableLabel, 10);
            ...
            newXQueryVariableButton.addListener(SWT.Selection, new Listener() {
                public void handleEvent(Event event) {
                    TableItem item = bindXQueryVariable(EsbFactory.eINSTANCE.createXQueryVariable());
        ...
        removeXQueryVariableButton = new Button(container, SWT.NONE);
        {
            ...
            removeXQueryVariableButtonLayoutData.top = new FormAttachment(newXQueryVariableButton, 5);
            removeXQueryVariableButtonLayoutData.right = new FormAttachment(100);
            removeXQueryVariableButtonLayoutData.left = new FormAttachment(newXQueryVariableButton, 0, SWT.LEFT);
            removeXQueryVariableButton.setLayoutData(removeXQueryVariableButtonLayoutData);
        ...
        xqueryVariableTable = new Table(container, SWT.BORDER | SWT.FULL_SELECTION | SWT.HIDE_SELECTION);
        {
            TableColumn nameColumn = new TableColumn(xqueryVariableTable, SWT.LEFT);
            TableColumn variableTypeColumn = new TableColumn(xqueryVariableTable, SWT.LEFT);
            TableColumn valueColumn = new TableColumn(xqueryVariableTable, SWT.LEFT);
            TableColumn typeColumn = new TableColumn(xqueryVariableTable, SWT.LEFT);
            ...
            xqueryVariableTable.addListener(SWT.Selection, tblPropertiesListener);
            ...
            // Layout.
            FormData xqueryVariableTableLayoutData = new FormData(SWT.DEFAULT, 150);
            xqueryVariableTableLayoutData.top = new FormAttachment(newXQueryVariableButton, 0, SWT.TOP);
            xqueryVariableTableLayoutData.left = new FormAttachment(0);
            xqueryVariableTableLayoutData.right = new FormAttachment(newXQueryVariableButton, -5);
    ...
    private void editItem(final TableItem item) {
        ...
        variableTypeEditor = initTableEditor(variableTypeEditor, item.getParent());
        cmbVariableType = new Combo(item.getParent(), SWT.READ_ONLY);
        cmbVariableType.setItems(new String[] { "DOCUMENT", "DOCUMENT_ELEMENT", "ELEMENT", "INT", "INTEGER", "BOOLEAN",
                "BYTE", "DOUBLE", "SHORT", "LONG", "FLOAT", "STRING" });
        ...
        variableValueEditor = initTableEditor(variableValueEditor, item.getParent());
    ...
    private TableItem bindXQueryVariable(XQueryVariable variable) {
        ...
        if (variable.getValueType().getLiteral().equals("LITERAL")) {
            item.setText(new String[] { variable.getVariableName(), variable.getVariableType().getLiteral(),
                    variable.getValueLiteral(), variable.getValueType().getLiteral() });
        }
        if (variable.getValueType().getLiteral().equals("EXPRESSION")) {
            item.setText(new String[] { variable.getVariableName(), variable.getVariableType().getLiteral(),
                    variable.getValueExpression().getPropertyValue(), variable.getValueType().getLiteral(),
                    variable.getValueKey().getKeyValue() });
        }

        item.setData(variable);
        item.setData("exp", EsbFactory.eINSTANCE.copyNamespacedProperty(variable.getValueExpression()));
        return item;
    }
    ...
    private void unbindXQueryVariable(int itemIndex) {
        ...
        if (null != variable.eContainer()) {
            RemoveCommand removeCmd = new RemoveCommand(editingDomain, xqueryMediator,
        ...
        for (TableItem item : xqueryVariableTable.getItems()) {
            ...
            if (null == variable.eContainer()) {
                ...
                if (!variable.getVariableName().equals(item.getText(0))) {
                    SetCommand setCmd = new SetCommand(editingDomain, variable,
                            EsbPackage.Literals.XQUERY_VARIABLE__VARIABLE_NAME, item.getText(0));
                    getResultCommand().append(setCmd);
                }

                if (item.getText(3).equals("LITERAL")) {

                    SetCommand setCmdValueType = new SetCommand(editingDomain, variable,
                            EsbPackage.Literals.XQUERY_VARIABLE__VALUE_TYPE, XQueryVariableValueType.LITERAL);
                    getResultCommand().append(setCmdValueType);
                    if (!variable.getValueLiteral().equals(item.getText(2))) {
                        SetCommand setCmd = new SetCommand(editingDomain, variable,
                                EsbPackage.Literals.XQUERY_VARIABLE__VALUE_LITERAL, item.getText(2));
                        getResultCommand().append(setCmd);
                    }
                }
                if (item.getText(3).equals("EXPRESSION")) {
                    SetCommand setCmdValueType = new SetCommand(editingDomain, variable,
                            EsbPackage.Literals.XQUERY_VARIABLE__VALUE_TYPE, XQueryVariableValueType.EXPRESSION);
                    getResultCommand().append(setCmdValueType);
                    ...
                    if (variable.getValueExpression() == null) {
                        ...
                        AddCommand addCmd = new AddCommand(editingDomain, variable,
                                EsbPackage.Literals.XQUERY_VARIABLE__VALUE_EXPRESSION, namespaceProperty);
                        getResultCommand().append(addCmd);
                    } else {
                        SetCommand setCmd = new SetCommand(editingDomain, variable.getValueExpression(),
                                EsbPackage.Literals.NAMESPACED_PROPERTY__PROPERTY_VALUE, item.getText(2));
                        getResultCommand().append(setCmd);

                        setCmd = new SetCommand(editingDomain, variable.getValueExpression(),
                    ...
                    if (variable.getValueKey() == null) {
                        ...
                        AddCommand addCmd = new AddCommand(editingDomain, variable,
                                EsbPackage.Literals.XQUERY_VARIABLE__VALUE_KEY, keyProperty);
                        getResultCommand().append(addCmd);

                    } else {
                        SetCommand setCmd = new SetCommand(editingDomain, variable.getValueKey(),
                                EsbPackage.Literals.REGISTRY_KEY_PROPERTY__KEY_VALUE, item.getText(4));
                        getResultCommand().append(setCmd);
                    }
                ...
                if (item.getText(1).equals("STRING"))
                    xqueryVariableType = XQueryVariableType.STRING;
                SetCommand setCmdValueType = new SetCommand(editingDomain, variable,
                        EsbPackage.Literals.XQUERY_VARIABLE__VARIABLE_TYPE, xqueryVariableType);
                getResultCommand().append(setCmdValueType);
