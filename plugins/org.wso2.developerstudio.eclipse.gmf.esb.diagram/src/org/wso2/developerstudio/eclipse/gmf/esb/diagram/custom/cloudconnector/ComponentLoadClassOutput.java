    ...
    protected void deserialize(OMElement documentElement) throws Exception {
        ...
        for (OMElement omElement : artifactsElements) {
            ...
            for (OMElement omElement2 : artifactElements) {
                if (omElement2.getChildrenWithLocalName("connectorSchemaGeneratorOutput").hasNext()) {
                    OMElement loadClassElement = (OMElement) omElement2
                            .getChildrenWithLocalName("connectorSchemaGeneratorOutput").next();
                    SubComponents artifactDependency = new SubComponents(
                            omElement2.getAttributeValue(new QName("name")), loadClassElement.getText());
