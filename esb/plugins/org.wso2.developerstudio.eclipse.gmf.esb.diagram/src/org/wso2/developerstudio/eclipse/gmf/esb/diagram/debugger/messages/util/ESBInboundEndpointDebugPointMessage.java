 ...
 *
 */
public class ESBInboundEndpointDebugPointMessage extends
		AbstractESBDebugPointMessage {

	private ESBInboundEndpointSequenceBean sequence;

	public ESBInboundEndpointDebugPointMessage(String command,
			String commandArgument, ESBInboundEndpointSequenceBean sequence) {
		super(command, commandArgument, SEQUENCE_LABEL);
		this.setSeqeunce(sequence);
	}

	public ESBInboundEndpointDebugPointMessage(EventMessageType event,
	...
	public boolean equalsIgnoreType(
			ESBInboundEndpointDebugPointMessage debugPointMessage) {
	...
	public ESBMediatorPosition getMediatorPosition() {
		return sequence.getInboundEndpoint().getMediatorPosition();
	}

	@Override
	public void setMediatorPosition(List<Integer> positionList) {
		sequence.getInboundEndpoint().setMediatorPosition(
				new ESBMediatorPosition(positionList));
	}

	@Override
	public String getSequenceType() {
		return sequence.getInboundEndpoint().getSequenceType();
	}

	private void createInboundSequenceFromJsonElement(
			JsonElement recievedArtifactInfo) {
		...
		for (Entry<String, JsonElement> inboundEntry : entrySet) {
			...
			Set<Entry<String, JsonElement>> inboundEntrySet = inboundArtifactInfo
					.getAsJsonObject().entrySet();
			for (Entry<String, JsonElement> entry : inboundEntrySet) {
				if (INBOUND_ENDPOINT_KEY_LABEL.equalsIgnoreCase(entry.getKey())) {
					inboundKey = convertJsonElementValueToString(entry
							.getValue());
		...
		ESBInboundEndpointBean inbound = new ESBInboundEndpointBean(inboundKey,
				sequenceType, mediatorPosition);
		sequence = new ESBInboundEndpointSequenceBean(inbound);
	}
