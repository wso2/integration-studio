...
import org.wso2.integrationstudio.datamapper.impl.CeliImpl;
import org.wso2.integrationstudio.datamapper.impl.OperatorImpl;

public class CeilingModelTransformer implements IModelTransformer {

	@Override
	public DMOperation transform(OperatorImpl operator, int index) {
		if (operator instanceof CeliImpl) {
			DMOperation ceilingOperatorModel = new DMOperation(DataMapperOperatorType.CEILING, operator.toString(),
					index);
			return ceilingOperatorModel;
		}
		throw new IllegalArgumentException("CeliImpl operator expected. Found : " + operator.toString());
	}
