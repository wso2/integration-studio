...
public class ConfigureCommandMediatorDialog extends Dialog {
    ...
    public ConfigureCommandMediatorDialog(Shell parentShell, CommandMediator commandMediator,
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        newCommandPropertyButton = new Button(container, SWT.NONE);
        {
            ...
            FormData newCommandPropertyButtonLayoutData = new FormData(80, SWT.DEFAULT);
            newCommandPropertyButtonLayoutData.top = new FormAttachment(commandPropertyLabel, 10);
            ...
            newCommandPropertyButton.addListener(SWT.Selection, new Listener() {
                public void handleEvent(Event event) {
                    TableItem item = bindCommandProperty(EsbFactory.eINSTANCE.createCommandProperty());
        ...
        removeCommandPropertyButton = new Button(container, SWT.NONE);
        {
            ...
            removeCommandPropertyButtonLayoutData.top = new FormAttachment(newCommandPropertyButton, 5);
            removeCommandPropertyButtonLayoutData.right = new FormAttachment(100);
            removeCommandPropertyButtonLayoutData.left = new FormAttachment(newCommandPropertyButton, 0, SWT.LEFT);
            removeCommandPropertyButton.setLayoutData(removeCommandPropertyButtonLayoutData);
        ...
        commandPropertyTable = new Table(container, SWT.BORDER | SWT.FULL_SELECTION | SWT.HIDE_SELECTION);
        {
            TableColumn nameColumn = new TableColumn(commandPropertyTable, SWT.LEFT);
            TableColumn typeColumn = new TableColumn(commandPropertyTable, SWT.LEFT);
            TableColumn valueColumn = new TableColumn(commandPropertyTable, SWT.LEFT);
            TableColumn actionColumn = new TableColumn(commandPropertyTable, SWT.LEFT);
            ...
            commandPropertyTable.addListener(SWT.Selection, tblPropertiesListener);
            ...
            // Layout.
            FormData commandPropertyTableLayoutData = new FormData(SWT.DEFAULT, 150);
            commandPropertyTableLayoutData.top = new FormAttachment(newCommandPropertyButton, 0, SWT.TOP);
            commandPropertyTableLayoutData.left = new FormAttachment(0);
            commandPropertyTableLayoutData.right = new FormAttachment(newCommandPropertyButton, -5);
    ...
    private TableItem bindCommandProperty(CommandProperty property) {
        ...
        if (property.getValueType().equals(CommandPropertyValueType.MESSAGE_ELEMENT)) {
            if (null != property.getValueMessageElementXpath()) {
                item.setText(new String[] { property.getPropertyName(), MESSAGE_ELEMENT,
                        property.getValueMessageElementXpath().getPropertyValue(), getActionCaption(property) });
            }
            ...
            if (null != property.getValueContextPropertyName()) {
                item.setText(new String[] { property.getPropertyName(), CONTEXT_PROPERTY,
                        property.getValueContextPropertyName(), getActionCaption(property) });
            }
        } else {
            item.setText(new String[] { property.getPropertyName(), LITERAL, property.getValueLiteral() });
        }
    ...
    private void unbindCommandProperty(int itemIndex) {
        ...
        if (null != property.eContainer()) {
            RemoveCommand removeCmd = new RemoveCommand(editingDomain, commandMediator,
    ...
    private void editItem(final TableItem item) {
        ...
        if (!LITERAL.equalsIgnoreCase(item.getText(1))) {
            ...
            cmbAction.setItems(new String[] { READ, UPDATE, READ_AND_UPDATE });
            ...
            if (null == property.eContainer()) {
                ...
                if (!property.getPropertyName().equals(item.getText(0))) {
                    SetCommand setCmd = new SetCommand(editingDomain, property,
                            EsbPackage.Literals.COMMAND_PROPERTY__PROPERTY_NAME, item.getText(0));
                ...
                if (CONTEXT_PROPERTY.equalsIgnoreCase(item.getText(1))) {
                    ...
                    if (!property.getValueContextPropertyName().equals(item.getText(2))) {
                        setCmd = new SetCommand(editingDomain, property,
                                EsbPackage.Literals.COMMAND_PROPERTY__VALUE_CONTEXT_PROPERTY_NAME, item.getText(2));
                        getResultCommand().append(setCmd);
                    }
                    CommandPropertyContextAction contextAction = CommandPropertyContextAction.READ_CONTEXT;
                    if (READ_AND_UPDATE.equalsIgnoreCase(item.getText(3))) {
                    ...
                    setCmd = new SetCommand(editingDomain, property,
                            EsbPackage.Literals.COMMAND_PROPERTY__CONTEXT_ACTION, contextAction);
                    getResultCommand().append(setCmd);

                } else if (MESSAGE_ELEMENT.equalsIgnoreCase(item.getText(1))) {
                    SetCommand setCmd = new SetCommand(editingDomain, property,
                            EsbPackage.Literals.COMMAND_PROPERTY__VALUE_TYPE, CommandPropertyValueType.MESSAGE_ELEMENT);
                    getResultCommand().append(setCmd);
                    ...
                    if (property.getValueMessageElementXpath() == null) {
                        ...
                        SetCommand addCmd = new SetCommand(editingDomain, property,
                                EsbPackage.Literals.COMMAND_PROPERTY__VALUE_MESSAGE_ELEMENT_XPATH, namespaceProperty);
                        getResultCommand().append(addCmd);
                    } else {
                        setCmd = new SetCommand(editingDomain, property.getValueMessageElementXpath(),
                    ...
                    setCmd = new SetCommand(editingDomain, property,
                            EsbPackage.Literals.COMMAND_PROPERTY__MESSAGE_ACTION, messageAction);
                    getResultCommand().append(setCmd);
                } else {
                    SetCommand setCmd = new SetCommand(editingDomain, property,
                            EsbPackage.Literals.COMMAND_PROPERTY__VALUE_TYPE, CommandPropertyValueType.LITERAL);
                    getResultCommand().append(setCmd);

                    if (!property.getValueLiteral().equals(item.getText(2))) {
                        setCmd = new SetCommand(editingDomain, property,
                                EsbPackage.Literals.COMMAND_PROPERTY__VALUE_LITERAL, item.getText(2));
                        getResultCommand().append(setCmd);
                    }
                }
            }
