...
import org.wso2.integrationstudio.datamapper.impl.MultiplyImpl;
import org.wso2.integrationstudio.datamapper.impl.OperatorImpl;

public class MultiplyModelTransformer implements IModelTransformer {

	@Override
	public DMOperation transform(OperatorImpl operator, int index) {
		if (operator instanceof MultiplyImpl) {
			DMOperation multiplyOperatorModel = new DMOperation(DataMapperOperatorType.MULTIPLY, operator.toString(),
					index);
			return multiplyOperatorModel;
		}
		throw new IllegalArgumentException("MultiplyImpl operator expected. Found : " + operator.toString());
	}
