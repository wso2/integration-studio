...
import org.wso2.integrationstudio.datamapper.impl.SubtractImpl;

public class SubtractModelTransformer implements IModelTransformer {

	@Override
	public DMOperation transform(OperatorImpl operator, int index) {
		if (operator instanceof SubtractImpl) {
			DMOperation subtractOperatorModel = new DMOperation(DataMapperOperatorType.SUBTRACT, operator.toString(),
					index);
			return subtractOperatorModel;
		}
		throw new IllegalArgumentException("SubtractImpl operator expected. Found : " + operator.toString());
	}
