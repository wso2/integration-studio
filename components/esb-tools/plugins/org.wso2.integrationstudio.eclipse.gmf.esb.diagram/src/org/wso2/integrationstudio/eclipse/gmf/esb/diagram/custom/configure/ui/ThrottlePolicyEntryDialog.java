...
public class ThrottlePolicyEntryDialog extends Dialog {
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        policyEntryTable = new Table(container, SWT.BORDER | SWT.FULL_SELECTION | SWT.HIDE_SELECTION);
    ...
    private TableItem bindPolicyEntry(ThrottlePolicyEntry policyEntry) {
        ...
        item.setText(new String[] { policyEntry.getThrottleRange(), policyEntry.getThrottleType().getLiteral(),
                Integer.toString(policyEntry.getMaxRequestCount()), Integer.toString(policyEntry.getUnitTime()),
                Integer.toString(policyEntry.getProhibitPeriod()), policyEntry.getAccessType().getLiteral() });
        ...
        for (TableItem item : policyEntryTable.getItems()) {
            ...
            if (policyEntry.eContainer() == null) {
                ...
                if (!throttlePloicyType.equals(item.getText(1))) {
                    ...
                    SetCommand setCmd = new SetCommand(editingDomain, policyEntry,
                            EsbPackage.Literals.THROTTLE_POLICY_ENTRY__THROTTLE_TYPE, type);
                    getResultCommand().append(setCmd);
                }
                ...
                if (isInteger(item.getText(4))
                        && policyEntry.getProhibitPeriod() != Integer.parseInt(item.getText(4))) {

                    SetCommand setCmd = new SetCommand(editingDomain, policyEntry,
                            EsbPackage.Literals.THROTTLE_POLICY_ENTRY__PROHIBIT_PERIOD,
                            Integer.parseInt(item.getText(4)));
                    getResultCommand().append(setCmd);
                }
                ...
                if (!accessType.equals(item.getText(5))) {
                    ...
                    SetCommand setCmd = new SetCommand(editingDomain, policyEntry,
                            EsbPackage.Literals.THROTTLE_POLICY_ENTRY__ACCESS_TYPE, type);
                    getResultCommand().append(setCmd);
                }
            }
        }
