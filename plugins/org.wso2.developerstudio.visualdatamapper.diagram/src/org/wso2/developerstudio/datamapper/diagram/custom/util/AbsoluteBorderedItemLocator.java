...
public class AbsoluteBorderedItemLocator extends BorderItemLocator {
	/**
	 * Border item figure.
	 */
	 ...
	 */
	public AbsoluteBorderedItemLocator(IFigure parentFigure, IFigure borderItemFigure, int side, double locationScale) {
	...
	protected Point locateOnBorder(Point suggestedLocation, int suggestedSide, int circuitCount, IFigure borderItem) {
		...
		if (suggestedSide == PositionConstants.WEST) {
			resultY = parentFigureY + ( locationScale);
		} else if (suggestedSide == PositionConstants.SOUTH) {
			resultX = parentFigureX + ( locationScale) - (borderItemSize.width / 2);
