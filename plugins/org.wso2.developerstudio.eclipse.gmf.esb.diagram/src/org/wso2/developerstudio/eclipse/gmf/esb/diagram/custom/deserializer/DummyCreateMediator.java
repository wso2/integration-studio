...
public class DummyCreateMediator {
	...
	private static Set<String> artifacts = new HashSet<>(Arrays.asList("api", "proxy", "endpoint", "inboundEndpoint",
			"localEntry", "messageProcessor", "messageStore", "task", "template"));

	private static Set<String> similar = new HashSet<>(Arrays.asList("sequence"));

	/**
	 * InMediatorFactory.class, OutMediatorFactory.class, 
	 * AnnotatedCommandMediatorFactory.class, EventPublisherMediatorFactory.class, 
	 ...
	 * TemplateMediatorFactory.class, CommentMediatorFactory.class
	 */
	...
	private Mediator createMediatorObject(OMElement omElement, String localName) {
		...
		if ("log".equals(localName)) {
			...
			mediator = factory.createSpecificMediator(omElement);
