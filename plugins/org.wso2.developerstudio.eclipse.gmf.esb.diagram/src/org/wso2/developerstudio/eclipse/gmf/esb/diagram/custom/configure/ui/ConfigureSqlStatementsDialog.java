...
public class ConfigureSqlStatementsDialog extends Dialog {
     ...
     * @param editingDomain
     */
    public ConfigureSqlStatementsDialog(Shell parentShell, AbstractSqlExecutorMediator sqlExecutorMediator,
        ...
        cmdAddParameter.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                ...
                if (-1 != selectedIndex) {
                    ...
                    paramWrapper.setValueExpression(
                            EsbFactory.eINSTANCE.copyNamespacedProperty(parameter.getValueExpression()));
    ...
    private TableItem bindStatement(StatementWrapper wrapper) {
        ...
        for (SqlParameterDefinition parameter : statement.getParameters()) {
            ...
            paramWrapper.setDataType(parameter.getDataType());
            paramWrapper
                    .setValueExpression(EsbFactory.eINSTANCE.copyNamespacedProperty(parameter.getValueExpression()));
            paramWrapper.setValueLiteral(parameter.getValueLiteral());
            paramWrapper.setExpression(parameter.getValueType() == SqlParameterValueType.EXPRESSION);
            wrapper.getParameters().add(paramWrapper);
        }
    ...
    private void unbindStatement(int itemIndex) {
        ...
        if (null != statement.eContainer()) {
            ...
            RemoveCommand removeCmd = new RemoveCommand(editingDomain, sqlExecutorMediator,
                    EsbPackage.Literals.ABSTRACT_SQL_EXECUTOR_MEDIATOR__SQL_STATEMENTS, statement);
    ...
    private void editParameter(final TableItem item) {
        ...
        parameterTypeEditor = initTableEditor(parameterTypeEditor, item.getParent());
    ...
    protected void okPressed() {
        ...
        for (TableItem item : tblStatements.getItems()) {
            ...
            if (null == statement.eContainer()) {
                addCmd = new AddCommand(editingDomain, sqlExecutorMediator,
                        EsbPackage.Literals.ABSTRACT_SQL_EXECUTOR_MEDIATOR__SQL_STATEMENTS, statement);
            ...
            if (!statement.getQueryString().equals(wrapper.getQueryString())) {
                setCommand = new SetCommand(editingDomain, statement, EsbPackage.Literals.SQL_STATEMENT__QUERY_STRING,
            ...
            for (ParameterWrapper parmWrapper : wrapper.getParameters()) {
                ...
                if (null == parameter.eContainer()) {
                    addCmd = new AddCommand(editingDomain, statement, EsbPackage.Literals.SQL_STATEMENT__PARAMETERS,
                            parameter);
                    getResultCommand().append(addCmd);
                }

                if ((parameter.getValueType() == SqlParameterValueType.EXPRESSION) != parmWrapper.isExpression()) {
                    setCommand = new SetCommand(editingDomain, parameter,
                            EsbPackage.Literals.SQL_PARAMETER_DEFINITION__VALUE_TYPE,
                            parmWrapper.isExpression() ? SqlParameterValueType.EXPRESSION
                                    : SqlParameterValueType.LITERAL);
                    getResultCommand().append(setCommand);
                }
                ...
                if (!parameter.getValueLiteral().equals(parmWrapper.getValueLiteral())) {
                    setCommand = new SetCommand(editingDomain, parameter,
                            EsbPackage.Literals.SQL_PARAMETER_DEFINITION__VALUE_LITERAL, parmWrapper.getValueLiteral());
                    getResultCommand().append(setCommand);
                }

                if (!parameter.getDataType().equals(parmWrapper.getDataType())) {
                    setCommand = new SetCommand(editingDomain, parameter,
                            EsbPackage.Literals.SQL_PARAMETER_DEFINITION__DATA_TYPE, parmWrapper.getDataType());
                    getResultCommand().append(setCommand);
                }
            }
            ...
            for (ResultWrapper resultWrapper : wrapper.getResults()) {
                ...
                if (null == result.eContainer()) {
                    addCmd = new AddCommand(editingDomain, statement, EsbPackage.Literals.SQL_STATEMENT__RESULTS,
                            result);
                    getResultCommand().append(addCmd);
                }
                ...
                if (!result.getPropertyName().equals(resultWrapper.getPropertyName())) {
                    setCommand = new SetCommand(editingDomain, result,
                            EsbPackage.Literals.SQL_RESULT_MAPPING__PROPERTY_NAME, resultWrapper.getPropertyName());
                    getResultCommand().append(setCommand);
                }

                if (!result.getColumnId().equals(resultWrapper.getColumnId())) {
                    setCommand = new SetCommand(editingDomain, result,
                            EsbPackage.Literals.SQL_RESULT_MAPPING__COLUMN_ID, resultWrapper.getColumnId());
                    getResultCommand().append(setCommand);
                }
            }
