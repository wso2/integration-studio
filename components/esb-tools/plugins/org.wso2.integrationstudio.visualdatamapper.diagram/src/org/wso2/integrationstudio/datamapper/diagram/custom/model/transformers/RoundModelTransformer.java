...
import org.wso2.integrationstudio.datamapper.impl.RoundImpl;

public class RoundModelTransformer implements IModelTransformer {

	@Override
	public DMOperation transform(OperatorImpl operator, int index) {
		if (operator instanceof RoundImpl) {
			DMOperation roundOperatorModel = new DMOperation(DataMapperOperatorType.ROUND, operator.toString(), index);
			return roundOperatorModel;
		}
		throw new IllegalArgumentException("RoundImpl operator expected. Found : " + operator.toString());
	}
