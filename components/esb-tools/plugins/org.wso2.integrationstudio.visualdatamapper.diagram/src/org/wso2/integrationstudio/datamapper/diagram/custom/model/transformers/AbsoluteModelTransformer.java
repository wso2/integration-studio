...
import org.wso2.integrationstudio.datamapper.impl.AbsoluteValueImpl;
import org.wso2.integrationstudio.datamapper.impl.OperatorImpl;

public class AbsoluteModelTransformer implements IModelTransformer {

	@Override
	public DMOperation transform(OperatorImpl operator, int index) {
		if (operator instanceof AbsoluteValueImpl) {
			DMOperation absoluteValueOperatorModel = new DMOperation(DataMapperOperatorType.ABSOLUTE,
					operator.toString(), index);
			return absoluteValueOperatorModel;
		}
		throw new IllegalArgumentException("AbsoluteValueImpl operator expected. Found : " + operator.toString());
	}
