...
public class ConfigureOROperatorDialog extends AbstractConfigureOperatorDialog {
	...
	public ConfigureOROperatorDialog(Shell parentShell, OR selectedObj, TransactionalEditingDomain editingDomain) {
		super(parentShell);
		this.orImpl = (OR) selectedObj;
	...
	public void create() {
		super.create();
		setTitle("Configure OR Operator");
		setMessage("Set or operator properties", IMessageProvider.INFORMATION);
	}

	protected void configureShell(Shell newShell) {
		super.configureShell(newShell);
		// Set title.
		newShell.setText("Configure OR Operator");
	}
	...
	protected Control createDialogArea(Composite parent) {
		...
		inputConnectorCount.setText(orImpl.getBasicContainer().getLeftContainer().getLeftConnectors().size() + "");
	...
	protected void okPressed() {
		...
		int number = Integer.parseInt(inputCount)
				- orImpl.getBasicContainer().getLeftContainer().getLeftConnectors().size();
		if (number > 0) {
			for (int i = 0; i < number; ++i) {
				OperatorLeftConnector addOperatorContainers = DataMapperFactory.eINSTANCE.createOperatorLeftConnector();
				AddCommand addCmd = new AddCommand(editingDomain, orImpl.getBasicContainer().getLeftContainer(),
			...
			for (int i = 0; i < Math.abs(number); i++) {
				EList<OperatorLeftConnector> listOfLeftConnectors = orImpl.getBasicContainer().getLeftContainer()
						.getLeftConnectors();
