...
public class IntegrationStudioProviderUtils {
	...
	public static boolean isInProjectFilterPassed(Map<String, List<String>> filters, IResource registryResource) {
		boolean passed = true;
		if (IntegrationStudioProviderUtils.isFilterPresent(filters, CSProviderConstants.FILTER_PROJECT)) {
			String projectName = registryResource.getProject().getName();
			passed = false;
			List<String> filterValues =
			                            IntegrationStudioProviderUtils.getFilterValues(filters,
	...
	public static boolean isMediaTypeFilterPassed(Map<String, List<String>> filters, File file) {
		boolean passed = true;
		try {
			if (IntegrationStudioProviderUtils.isFilterPresent(filters, CSProviderConstants.FILTER_MEDIA_TYPE)) {
				String mediaType = MediaManager.getMediaType(file);
				passed = IntegrationStudioProviderUtils.isMediaTypeFilterPassed(filters, mediaType);
			}
		} catch (Exception e) {
			passed = false;
		}
