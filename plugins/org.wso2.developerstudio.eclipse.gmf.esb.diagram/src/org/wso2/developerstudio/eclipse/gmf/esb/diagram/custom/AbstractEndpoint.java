...
public abstract class AbstractEndpoint extends BorderedBorderItemEditPart implements DroppableElement {
    ...
    public void reverse(EditPart editorPart) {
        if (!reversed & (/* (editorPart.getParent() instanceof EndpointFlowEndpointCompartmentEditPart)&& */(editorPart
            ...
            for (int i = 0; i < selectedEP.getChildren().size(); ++i) {
                if (selectedEP.getChildren().get(i) instanceof AbstractEndpointInputConnectorEditPart) {
                    ...
                    NodeFigure figureInput = ((AbstractEndpointInputConnectorEditPart) selectedEP.getChildren().get(i))
                            .getNodeFigureInput();
                ...
                if (selectedEP.getChildren().get(i) instanceof AbstractEndpointOutputConnectorEditPart) {
                    ...
                    collection.add(
                            ((org.eclipse.gmf.runtime.notation.impl.NodeImpl) ((AbstractEndpointOutputConnectorEditPart) selectedEP
                                    .getChildren().get(i)).getModel()).getElement());
                ...
                if (selectedEP.getChildren().get(i) instanceof AbstractEndpointAdditionalOutputConnectorEditPart) {
                    additionalOutputConnector = ((AbstractEndpointAdditionalOutputConnectorEditPart) selectedEP
                            .getChildren().get(i)).getFigure();
                    additionalOutputConnectors.add(additionalOutputConnector);

                    /*
                     * additionalOutputConnector = ((AbstractEndpointAdditionalOutputConnector)
                     * selectedEP.getChildren().get(i))
                     * .getFigure();
                     * additionalOutputConnectors.add(additionalOutputConnector);
            ...
            for (int j = 0; j < inputConnectors.size(); ++j) {
                ...
                selectedEP.getBorderedFigure().getBorderItemContainer().add(inputConnectors.get(j),
                        inputLocators.get(j));
