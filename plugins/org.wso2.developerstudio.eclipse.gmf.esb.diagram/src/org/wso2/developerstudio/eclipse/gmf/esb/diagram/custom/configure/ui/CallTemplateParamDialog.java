...
public class CallTemplateParamDialog extends Dialog {
    ...
    public CallTemplateParamDialog(Shell parentShell, Mediator mediator, TransactionalEditingDomain editingDomain) {
        super(parentShell);
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        newParamButton.addListener(SWT.Selection, new Listener() {
            public void handleEvent(Event event) {
                TableItem item = bindPram(EsbFactory.eINSTANCE.createCallTemplateParameter());
    ...
    private TableItem bindPram(CallTemplateParameter param) {
        ...
        if (param.getTemplateParameterType().getLiteral().equals(VALUE_TYPE)) {
            item.setText(new String[] { param.getParameterName(), param.getTemplateParameterType().getLiteral(),
            ...
            if (param.eContainer() == null) {
                ...
                if (!param.getParameterName().equals(item.getText(0))) {
                    ...
                    SetCommand setCmd = new SetCommand(editingDomain, param,
                            EsbPackage.Literals.CALL_TEMPLATE_PARAMETER__PARAMETER_NAME, item.getText(0));
                ...
                if (item.getText(1).equals(VALUE_TYPE)) {
                    ...
                    SetCommand setCmdValueType = new SetCommand(editingDomain, param,
                            EsbPackage.Literals.CALL_TEMPLATE_PARAMETER__TEMPLATE_PARAMETER_TYPE, RuleOptionType.VALUE);
                    getResultCommand().append(setCmdValueType);

                    if (!param.getParameterValue().equals(item.getText(2))) {

                        SetCommand setCmd = new SetCommand(editingDomain, param,
                                EsbPackage.Literals.CALL_TEMPLATE_PARAMETER__PARAMETER_VALUE, item.getText(2));
                        getResultCommand().append(setCmd);
                    }
                }
                ...
                if (item.getText(1).equals(EXPRESSION_TYPE)) {
                    ...
                    SetCommand setCmdExpType = new SetCommand(editingDomain, param,
                    ...
                    if (param.getParameterExpression() == null) {
                        ...
                        namespaceProperty.setNamespaces(expression.getNamespaces());
                        namespaceProperty.setDynamic(expression.isDynamic());
                        AddCommand addCmd = new AddCommand(editingDomain, param,
