...
public class CustomPaletteViewerKeyHandler extends PaletteViewerKeyHandler {
    ...
    private void filterMatchingEntries(KeyEvent event) {
        ...
        EsbServer server = ((EsbDiagram) ((EsbDiagramEditor) esbMultiPageEditor.getGraphicalEditor())
                .getDiagramEditPart().getDiagramView().getElement()).getServer();

        if (getFocusEditPart() instanceof DrawerEditPart && ((DrawerEditPart) getFocusEditPart()).isExpanded()) {
            ...
            for (Object child : children) {
                if (child instanceof DrawerEditPart && ((PaletteDrawer) ((DrawerEditPart) child).getModel()).isVisible()
                        && ((DrawerEditPart) child).isExpanded()) {
                    PaletteDrawer paletteDrawer = (PaletteDrawer) ((DrawerEditPart) child).getModel();
                    if (!paletteDrawer.getLabel().equals("Links")
                            && paletteEntries.get(paletteDrawer.getLabel()) != null) {
        ...
        if (paletteEntryList != null && paletteEntryList.size() > 0) {
            ...
            for (int i = 0; i < paletteEntryList.size(); i++) {
                if (paletteEntryList.get(i) instanceof PaletteToolEntry) {
                    ...
                    if (!(ArtifactType.INBOUND_ENDPOINT.equals(server.getType())
                            && "Mediators".equals(toolEntry.getParent().getLabel()))) {
