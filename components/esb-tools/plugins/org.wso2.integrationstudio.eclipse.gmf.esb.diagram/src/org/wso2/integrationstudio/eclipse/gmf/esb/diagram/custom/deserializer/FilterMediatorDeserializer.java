...
public class FilterMediatorDeserializer extends AbstractEsbNodeDeserializer<AbstractMediator, FilterMediator> {
    ...
    public FilterMediator createNode(IGraphicalEditPart part, AbstractMediator mediator) throws DeserializerException {
        ...
        Assert.isTrue(mediator instanceof org.apache.synapse.mediators.filters.FilterMediator,
        ...
        FilterMediator visualFilter = (FilterMediator) DeserializerUtils.createNode(part,
        ...
        if (filterMediator.getSource() != null && filterMediator.getRegex() != null) {
            ...
            if (filterMediator.getRegex() != null
                    && DeserializerUtils.isValidRegex(filterMediator.getRegex().toString())) {
        ...
        if (filterMediator.getElseMediator() != null) {
            ...
            IGraphicalEditPart compartment = (IGraphicalEditPart) getEditpart(
                    visualFilter.getFilterContainer().getFailContainer().getMediatorFlow()).getChildren().get(0);
            deserializeSequence(compartment, sequence, visualFilter.getFailOutputConnector());
        }
        ...
        if (filterMediator.getList().size() > 0) {
            ...
            IGraphicalEditPart compartment = (IGraphicalEditPart) getEditpart(
                    visualFilter.getFilterContainer().getPassContainer().getMediatorFlow()).getChildren().get(0);
            deserializeSequence(compartment, sequence, visualFilter.getPassOutputConnector());
        }
