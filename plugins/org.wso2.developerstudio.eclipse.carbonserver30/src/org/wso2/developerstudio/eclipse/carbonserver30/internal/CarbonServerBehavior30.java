...
public class CarbonServerBehavior30 extends CarbonServerBehaviour {
	...
	private void doBrowserPopup() {
		...
		if (popupBrowser != null && popupBrowser) {
			...
			uploadServiceRequestUtil.popupExternalBrowser(CommonOperations.getLocalServerPort(getServer()) +
			                                              CarbonServer30Utils.getWebContextRoot(getServer()) +
			                                              "/carbon");
		}
	...
	protected String getTransportXmlFilePath() {
		...
		String transportsXmlPath =
		                           CarbonServer30Utils.getTransportsXmlPathFromLocalWorkspaceRepo(serverHome.toOSString());
