...
public class ThrottleOnAcceptBranchPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
     ...
     * 
     */
    public ThrottleOnAcceptBranchPropertiesEditionComponent(PropertiesEditingContext editingContext,
            EObject throttleOnAcceptBranch, String editing_mode) {
        ...
        if (editingPart != null && key == partKey) {
            ...
            if (isAccessible(EsbViewsRepository.ThrottleOnAcceptBranch.Properties.sequenceType)) {
                basePart.initSequenceType(
                        EEFUtils.choiceOfValues(throttleOnAcceptBranch,
                                EsbPackage.eINSTANCE.getThrottleOnAcceptBranch_SequenceType()),
                        throttleOnAcceptBranch.getSequenceType());
            }
            // init filters
    ...
    public void updatePart(Notification msg) {
        ...
        if (editingPart.isVisible()) {
            ...
            if (EsbPackage.eINSTANCE.getThrottleOnAcceptBranch_SequenceType().equals(msg.getFeature())
                    && msg.getNotifier().equals(semanticObject)
                    && isAccessible(EsbViewsRepository.ThrottleOnAcceptBranch.Properties.sequenceType))
                ...
                if (EsbViewsRepository.ThrottleOnAcceptBranch.Properties.sequenceType == event.getAffectedEditor()) {
                    ...
                    if (newValue instanceof String) {
                        newValue = EEFConverterUtil.createFromString(
                                EsbPackage.eINSTANCE.getThrottleOnAcceptBranch_SequenceType().getEAttributeType(),
                                (String) newValue);
                    }
