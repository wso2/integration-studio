...
public class ConfigureProxyWSDLResourceDialog extends Dialog {
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        newResourceButton.addListener(SWT.Selection, new Listener() {
            public void handleEvent(Event event) {
                TableItem item = bindPram(EsbFactory.eINSTANCE.createProxyWSDLResource());
        ...
        removeWSDLResourceButtonLayoutData.top = new FormAttachment(newResourceButton, 5);
        removeWSDLResourceButtonLayoutData.right = new FormAttachment(100);
        removeWSDLResourceButtonLayoutData.left = new FormAttachment(newResourceButton, 0, SWT.LEFT);
        ...
        logPropertiesTableLayoutData.top = new FormAttachment(newResourceButton, 0, SWT.TOP);
        logPropertiesTableLayoutData.left = new FormAttachment(0);
        logPropertiesTableLayoutData.right = new FormAttachment(newResourceButton, -5);
    ...
    private void unbindParam(int itemIndex) {
        ...
        if (param.eContainer() != null) {
            RemoveCommand removeCmd = new RemoveCommand(editingDomain, proxyService,
                    EsbPackage.Literals.PROXY_SERVICE__WSDL_RESOURCES, param);
    ...
    protected void okPressed() {
        ...
        for (TableItem item : resourcesTable.getItems()) {
            ...
            ProxyWSDLResource resource = (ProxyWSDLResource) item.getData();

            if (resource.eContainer() == null) {

                // SetCommand setCmd=new SetCommand(editingDomain, proxyService,
                // EsbPackage.Literals.PROXY_SERVICE__WSDL_RESOURCES, resource);
                ...
                if (!resource.getKey().getKeyValue().equals(item.getText(0))) {
                    ...
                    SetCommand setCmd = new SetCommand(editingDomain, resource,
                            EsbPackage.Literals.PROXY_SERVICE__WSDL_RESOURCES, item.getText(0));
                ...
                if (!resource.getLocation().equals(item.getText(1))) {
                    ...
                    SetCommand setCmd = new SetCommand(editingDomain, resource,
                            EsbPackage.Literals.PROXY_SERVICE__WSDL_RESOURCES, item.getText(1));
