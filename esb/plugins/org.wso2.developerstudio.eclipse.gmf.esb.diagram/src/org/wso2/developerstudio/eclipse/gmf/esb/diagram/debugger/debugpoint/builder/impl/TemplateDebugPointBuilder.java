 ...
 * <p>
 * If the affected mediator of debug point related action is in a Sequence
 * Template, this builder class can be use directly or ask from
 * {@link ESBDebugPointBuilderFactory} for a instance of this class to create or
 * modify the debug point.
 */
	...
	public ESBDebugPoint getESBDebugPoint(EsbServer esbServer,
		...
		if (template.getChild() instanceof SequencesImpl) {
			...
			ESBTemplateDebugPointMessage templateDebugPoint = new ESBTemplateDebugPointMessage(
					null, commandArgument, TEMPLATE_LABEL, templateBean);
	...
	public void updateExistingDebugPoints(IResource resource,
		...
		if (template.getChild() instanceof SequencesImpl) {
			...
			List<ESBDebugPoint> breakpontList = getDebugPointsRelatedToModification(
					resource, position, TEMPLATE_SEQUENCE, action);
