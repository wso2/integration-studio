...
public class DataMapperDiagramModel {
	...
	private List<DMOperation> operationsArray = new ArrayList<>();
	...
	private void updateExecutionSequence() {
		...
		int numberOfOperations = operationsArray.size();
		...
		while (executionSeq.size() < numberOfOperations) {
			for (int index = 0; unexecutedOperationList.size() > index; ++index) {
				if (operationIsExecutable(unexecutedOperationList.get(index))) {
					...
					addOutputsToResolvedVariables(unexecutedOperationList
							.get(index));
				} else {
					tempUnexecutedOperationList.add(unexecutedOperationList
							.get(index));
				}
			}
			...
			if (objectElement instanceof ElementImpl) {
				...
				if (element.getLevel() <= parentVariableStack.size()) {
					...
					throw new IllegalArgumentException(
							"Illegal element level detected : element level- "
									+ element.getLevel() + " , parents level- "
									+ parentVariableStack.size());
				}
				int index = variablesArray.size();
				variablesArray
						.add(new DMVariable(getVariableName(
								DMVariableType.INPUT, parentVariableStack,
								((ElementImpl) objectElement).getName()),
								getUniqueId(objectElement),
								DMVariableType.INPUT, index));
	...
	private void populateAdjacencyLists(List<EObject> tempNodeArray) {
		while (tempNodeArray.size() > 0) {
			...
			if (nextElement instanceof ElementImpl) {
				EList<DataMapperLink> outgoingLinks = ((ElementImpl) nextElement)
				...
				for (DataMapperLink dataMapperLink : outgoingLinks) {
					...
					if (linkedNode instanceof ElementImpl) {
						operationsArray.add(new DMOperation(
								DMOperatorType.DIRECT, getUniqueDirectId(
										linkedNode, operationsArray.size()),
								operationsArray.size()));
						outputAdjList.add(new ArrayList<Integer>());
						outputAdjList.get(operationsArray.size() - 1).add(
								((ElementImpl) linkedNode).getIndex());
						inputAdjList.add(new ArrayList<Integer>());
						inputAdjList.get(operationsArray.size() - 1).add(
								((ElementImpl) nextElement).getIndex());
					} else if (linkedNode instanceof OperatorImpl
							&& !((OperatorImpl) linkedNode).isMarked()) {
						((OperatorImpl) linkedNode).setMarked(true);
						tempNodeArray.add(linkedNode);
					}
				}
			} else if (nextElement instanceof OperatorImpl
					&& !((OperatorImpl) nextElement).isVisited()) {
				int index = operationsArray.size();
				OperatorImpl operatorElement = (OperatorImpl) nextElement;
				DMOperation operator = new DMOperation(
						getOperatorType(operatorElement),
						getUniqueId(operatorElement), index);
				operationsArray.add(operator);
				...
				for (DataMapperLink dataMapperLink : outlinks) {
					...
					if (linkedElement instanceof ElementImpl) {
						outputAdjList.get(operatorElement.getIndex()).add(
								((ElementImpl) linkedElement).getIndex());
						...
						String variablePrefix = operator.getOperatorType()
								+ "_" + operator.getIndex() + "_" + indexOfLink;
						if (operatorElement.getPortVariableIndex().size() <= indexOfLink) {
							...
							DMVariable tempVar = new DMVariable(variablePrefix,
									getUniqueDirectId(operatorElement,
											indexOfLink),
									DMVariableType.INTERMEDIATE, variableIndex);
							variablesArray.add(tempVar);
							operatorElement.getPortVariableIndex().add(
									variableIndex);
							outputAdjList.get(operator.getIndex()).add(
									variableIndex);
							if (operatorElement.isVisited()) {
								inputAdjList.get(operatorElement.getIndex())
										.add(variableIndex);
							}
						}
					}
				...
				for (DataMapperLink dataMapperLink : inlinks) {
					...
					if (linkedElement instanceof ElementImpl) {
						inputAdjList.get(operatorElement.getIndex()).add(
								((ElementImpl) linkedElement).getIndex());
					} else if (linkedElement instanceof OperatorImpl) {
						...
						if (!sourceElement.isMarked()) {
							...
							int indexOfSourceLink = outLinksOfSourceElement
									.indexOf(dataMapperLink);
							inputAdjList.get(operator.getIndex()).add(
									sourceElement.getPortVariableIndex().get(
											indexOfSourceLink));
						}
					}
					indexOfPort++;
	...
	private List<DataMapperLink> getInLinksOfOperator(
			OperatorImpl operatorElement) {
		EList<OperatorLeftConnector> leftContainers = operatorElement
				.getBasicContainer().getLeftContainer().getLeftConnectors();
	...
	private List<DataMapperLink> getOutLinksOfOperator(
			OperatorImpl operatorElement) {
		EList<OperatorRightConnector> rightContainers = operatorElement
				.getBasicContainer().getRightContainer().getRightConnectors();
		List<DataMapperLink> linkList = new ArrayList<>();
		for (OperatorRightConnector operatorRightConnector : rightContainers) {
			linkList.addAll(operatorRightConnector.getOutNode()
					.getOutgoingLink());
		}
		return linkList;
	}
	...
	private void populateOutputVariables(Output output) {
		...
		for (Iterator<EObject> iterator = variableIterator; iterator.hasNext();) {
			...
			if (objectElement instanceof ElementImpl) {
				...
				int index = variablesArray.size();
				variablesArray.add(new DMVariable(getVariableName(
						DMVariableType.OUTPUT, parentVariableStack,
						((ElementImpl) objectElement).getName()), objectElement
						.toString(), DMVariableType.OUTPUT, index));
	...
	private String getVariableName(DMVariableType prefix,
			Stack<EObject> parentVariableStack, String name) {
		String variableName = prefix.toString().toLowerCase();
		for (EObject eObject : parentVariableStack) {
			if (eObject instanceof TreeNodeImpl) {
				if (!((TreeNodeImpl) eObject).getSchemaDataType().equals(
						SchemaDataType.ARRAY)) {
					variableName = variableName
							+ ((TreeNodeImpl) eObject).getName() + ".";
				}
				...
				throw new IllegalArgumentException(
						"Illegal element type found : " + eObject.toString());
			}
		}
		return variableName + name;
	}

	public List<DMOperation> getOperationsArray() {
	...
	public void setResolvedOutputVariableArray(
			List<Integer> resolvedOutputVariableArray) {
	...
	public List<Integer> getExecutionSeq() {
		return executionSeq;
	}
