...
public class AvroSchemagenPlugin extends Plugin {
	...
	private void generateAvroSchemas(JPackage rootPackage, Set<NamedAvroType> avroTypes) {
		...
		while (objectSize-index >= 0) {
			...
			String nameSpacename = avroElem.namespace.toString() + "." + avroElem.name.toString();
	...
	/**
	 * replace the name-spaces of the generated code from the actual name-spaces of the XSD 
	 * @param args
	 * @return
	 */
	public static String replaceNamespace(String insertedNamespace, String xsdFileLoc){
		...
		finalOutput = finalOutput.replace(existingNamespace, namespaceAttribute);
		return finalOutput;

	}

	
	public static void insertXSDAttrtoAvro(){
