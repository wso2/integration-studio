...
public class ValidateSchemasDialog extends Dialog {
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        schemaTable = new Table(container, SWT.BORDER | SWT.FULL_SELECTION | SWT.HIDE_SELECTION);
    ...
    private TableItem bindSchema(ValidateSchema schema) {
        ...
        item.setText(new String[] { schema.getValidateSchemaKeyType().toString(),
                schema.getValidateSchemaKeyType() == KeyType.STATIC ? schema.getValidateStaticSchemaKey().getKeyValue()
                        : schema.getValidateDynamicSchemaKey().getPropertyValue() });

        item.setData(schema);
        item.setData("staticKey", EsbFactory.eINSTANCE.copyRegistryKeyProperty(schema.getValidateStaticSchemaKey()));
        item.setData("dynamicKey", EsbFactory.eINSTANCE.copyNamespacedProperty(schema.getValidateDynamicSchemaKey()));
        return item;
    ...
    private void unbindSchema(int itemIndex) {
        ...
        if (null != schema.eContainer()) {
            ...
            RemoveCommand reoveCmd = new RemoveCommand(editingDomain, validateMediatoer,
            ...
            if (schema.eContainer() == null) {
                ...
                if (item.getText(0).equals(KeyType.STATIC.getLiteral())) {
                    ...
                    SetCommand setTypeCmd = new SetCommand(editingDomain, schema,
                            EsbPackage.Literals.VALIDATE_SCHEMA__VALIDATE_SCHEMA_KEY_TYPE, KeyType.STATIC);
                    ...
                    if (schema.getValidateStaticSchemaKey() != null
                        ...
                        SetCommand setCmd = new SetCommand(editingDomain, schema,
                                EsbPackage.Literals.VALIDATE_SCHEMA__VALIDATE_STATIC_SCHEMA_KEY, staticKey);
                        getResultCommand().append(setCmd);
                    }
                } else {

                    SetCommand setTypeCmd = new SetCommand(editingDomain, schema,
                            EsbPackage.Literals.VALIDATE_SCHEMA__VALIDATE_SCHEMA_KEY_TYPE, KeyType.DYNAMIC);
                    getResultCommand().append(setTypeCmd);
                    ...
                    if (schema.getValidateDynamicSchemaKey() != null
                        ...
                        SetCommand setCmd = new SetCommand(editingDomain, schema,
                                EsbPackage.Literals.VALIDATE_SCHEMA__VALIDATE_DYNAMIC_SCHEMA_KEY, dynamicKey);
                        getResultCommand().append(setCmd);
                    }
