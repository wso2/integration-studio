...
public class RouterMediatorDeserializer extends AbstractEsbNodeDeserializer<AbstractMediator, RouterMediator> {
    ...
    public RouterMediator createNode(IGraphicalEditPart part, AbstractMediator object) throws DeserializerException {
        ...
        RouterMediator routerModel = (RouterMediator) DeserializerUtils.createNode(part,
        ...
        for (Route route : routes) {
            ...
            if (count++ == 0 && routerModel.getRouterContainer().getRouterTargetContainer().size() == 1) {
                ...
                targetOutputConnector = (RouterMediatorTargetOutputConnector) DeserializerUtils.createNode(
                        (IGraphicalEditPart) getEditpart(routerModel),
                        EsbElementTypes.RouterMediatorTargetOutputConnector_3631);
            }
            executeSetValueCommand(targetContainer, ROUTER_TARGET_CONTAINER__BREAK_AFTER_ROUTE, route.isBreakRouter());
            ...
            executeSetValueCommand(targetContainer, ROUTER_TARGET_CONTAINER__ROUTE_PATTERN, route.getMatch().pattern());

            Target target = route.getTarget();
            RouterTarget routerTarget = targetContainer.getTarget();

            if (target.getSequence() != null) {
                executeSetValueCommand(routerTarget, ABSTRACT_COMMON_TARGET__SEQUENCE_TYPE,
                        TargetSequenceType.ANONYMOUS);
                refreshEditPartMap();
                IGraphicalEditPart compartment = (IGraphicalEditPart) getEditpart(targetContainer.getMediatorFlow())
                        .getChildren().get(0);
            ...
            if (target.getEndpoint() != null) {
                executeSetValueCommand(routerTarget, ABSTRACT_COMMON_TARGET__ENDPOINT_TYPE,
