...
public class RouterRoutePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
     ...
     * 
     */
    public RouterRoutePropertiesEditionComponent(PropertiesEditingContext editingContext, EObject routerRoute,
            String editing_mode) {
    ...
    public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
        ...
        if (editingPart != null && key == partKey) {
            ...
            if (isAccessible(EsbViewsRepository.RouterRoute.Properties.routeExpression)) {
                // init part
                routeExpressionSettings = new EObjectFlatComboSettings(routerRoute,
            ...
            if (isAccessible(EsbViewsRepository.RouterRoute.Properties.routePattern))
                basePart.setRoutePattern(
                        EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, routerRoute.getRoutePattern()));
    ...
    public void updateSemanticModel(final IPropertiesEditionEvent event) {
        ...
        if (EsbViewsRepository.RouterRoute.Properties.routePattern == event.getAffectedEditor()) {
            routerRoute.setRoutePattern((java.lang.String) EEFConverterUtil
    ...
    public void updatePart(Notification msg) {
        ...
        if (editingPart.isVisible()) {
            ...
            if (EsbPackage.eINSTANCE.getRouterRoute_BreakAfterRoute().equals(msg.getFeature())
                    && msg.getNotifier().equals(semanticObject) && basePart != null
                    && isAccessible(EsbViewsRepository.RouterRoute.Properties.breakAfterRoute))
                basePart.setBreakAfterRoute((Boolean) msg.getNewValue());

            if (EsbPackage.eINSTANCE.getRouterRoute_RouteExpression().equals(msg.getFeature()) && basePart != null
                    && isAccessible(EsbViewsRepository.RouterRoute.Properties.routeExpression))
                basePart.setRouteExpression((EObject) msg.getNewValue());
            if (EsbPackage.eINSTANCE.getRouterRoute_RoutePattern().equals(msg.getFeature())
                    && msg.getNotifier().equals(semanticObject) && basePart != null
                    && isAccessible(EsbViewsRepository.RouterRoute.Properties.routePattern)) {
                ...
                if (EsbViewsRepository.RouterRoute.Properties.breakAfterRoute == event.getAffectedEditor()) {
                    ...
                    if (newValue instanceof String) {
                        newValue = EEFConverterUtil.createFromString(
                                EsbPackage.eINSTANCE.getRouterRoute_BreakAfterRoute().getEAttributeType(),
                                (String) newValue);
                    }
                    ret = Diagnostician.INSTANCE.validate(
                            EsbPackage.eINSTANCE.getRouterRoute_BreakAfterRoute().getEAttributeType(), newValue);
                }
                if (EsbViewsRepository.RouterRoute.Properties.routePattern == event.getAffectedEditor()) {
                    Object newValue = event.getNewValue();
                    if (newValue instanceof String) {
                        newValue = EEFConverterUtil.createFromString(
                                EsbPackage.eINSTANCE.getRouterRoute_RoutePattern().getEAttributeType(),
                                (String) newValue);
                    }
                    ret = Diagnostician.INSTANCE
                            .validate(EsbPackage.eINSTANCE.getRouterRoute_RoutePattern().getEAttributeType(), newValue);
                }
