...
import org.wso2.integrationstudio.datamapper.impl.ToStringImpl;

public class ToStringModelTransformer implements IModelTransformer {

	@Override
	public DMOperation transform(OperatorImpl operator, int index) {
		if (operator instanceof ToStringImpl) {
			DMOperation toStringOperatorModel = new DMOperation(DataMapperOperatorType.TO_STRING, operator.toString(), index);
			return toStringOperatorModel;
		}
		throw new IllegalArgumentException("ToStringImpl operator expected. Found : " + operator.toString());
	}
}
