...
public class ConfigureXSLTMediatorDialog extends Dialog {
     ...
     * @param parentShell
     */
    public ConfigureXSLTMediatorDialog(Shell parentShell, XSLTMediator med, int activeTab) {
        super(parentShell);
        this.mediator = med;
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        tblFeatures = new Table(cFeatures, SWT.BORDER | SWT.CHECK | SWT.FULL_SELECTION);
        ...
        TableColumn colResourceLocation = new TableColumn(tableResources, SWT.NONE);
    ...
    protected void okPressed() {
        ...
        for (XSLTFeature feature : mediator.getFeatures()) {
            RemoveCommand removeCmd = new RemoveCommand(domain, mediator, EsbPackage.Literals.XSLT_MEDIATOR__FEATURES,
                    feature);
            getResultCommand().append(removeCmd);
        }

        for (XSLTProperty property : mediator.getProperties()) {
            RemoveCommand removeCmd = new RemoveCommand(domain, mediator, EsbPackage.Literals.XSLT_MEDIATOR__PROPERTIES,
                    property);
            getResultCommand().append(removeCmd);
        }

        for (XSLTResource resource : mediator.getResources()) {
            RemoveCommand removeCmd = new RemoveCommand(domain, mediator, EsbPackage.Literals.XSLT_MEDIATOR__RESOURCES,
                    resource);
            getResultCommand().append(removeCmd);
        }
         ...
         * SetCommand setCmd = new SetCommand(domain, mediator,EsbPackage.Literals.XSLT_MEDIATOR__SOURCE_XPATH,
         * sourceXPath);
         ...
         * SetCommand setRegKeyCmd = new SetCommand(domain,
         * mediator,EsbPackage.Literals.XSLT_MEDIATOR__XSLT_STATIC_SCHEMA_KEY, registryKey);
        ...
        for (TableItem item : tblProperties.getItems()) {
            ...
            if ("EXPRESSION".equalsIgnoreCase(item.getText(1))) {
                ...
                if (item.getData("exp") instanceof NamespacedProperty) {
                    property.setPropertyExpression(
                            EsbFactory.eINSTANCE.copyNamespacedProperty((NamespacedProperty) item.getData("exp")));
                } else {
            ...
            AddCommand addCmd = new AddCommand(domain, mediator, EsbPackage.Literals.XSLT_MEDIATOR__PROPERTIES,
                    property);
            getResultCommand().append(addCmd);
        }
        for (TableItem item : tableResources.getItems()) {
            ...
            AddCommand addCmd = new AddCommand(domain, mediator, EsbPackage.Literals.XSLT_MEDIATOR__RESOURCES,
                    resource);
            getResultCommand().append(addCmd);
        }
