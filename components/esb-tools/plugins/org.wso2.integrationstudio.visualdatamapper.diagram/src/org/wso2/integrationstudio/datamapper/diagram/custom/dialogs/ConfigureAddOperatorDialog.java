...
public class ConfigureAddOperatorDialog extends AbstractConfigureOperatorDialog {

	private String inputCount;
	private TransactionalEditingDomain editingDomain;
	private ArrayList<OperatorLeftConnector> caseOutputConnectors = new ArrayList<OperatorLeftConnector>();
	private AddImpl addImpl;

	public ConfigureAddOperatorDialog(Shell parentShell, Add addOperator, TransactionalEditingDomain editingDomain) {
		super(parentShell);
		this.addImpl = (AddImpl) addOperator;
	...
	public void create() {
		super.create();
		setTitle("Configure Add Operator");
		setMessage("Set add operator properties", IMessageProvider.INFORMATION);
	}

	protected void configureShell(Shell newShell) {
		super.configureShell(newShell);
		// Set title.
		newShell.setText("Configure Add Operator");
	}
	...
	protected Control createDialogArea(Composite parent) {
		...
		inputConnectorCount.setText(addImpl.getBasicContainer().getLeftContainer().getLeftConnectors().size() + "");
	...
	protected void okPressed() {
		...
		int number = Integer.parseInt(inputCount)
				- addImpl.getBasicContainer().getLeftContainer().getLeftConnectors().size();
		if (number > 0) {
			for (int i = 0; i < number; ++i) {
				OperatorLeftConnector addOperatorContainers = DataMapperFactory.eINSTANCE.createOperatorLeftConnector();
				AddCommand addCmd = new AddCommand(editingDomain, addImpl.getBasicContainer().getLeftContainer(),
			...
			for (int i = 0; i < Math.abs(number); i++) {
				EList<OperatorLeftConnector> listOfLeftConnectors = addImpl.getBasicContainer().getLeftContainer()
						.getLeftConnectors();
