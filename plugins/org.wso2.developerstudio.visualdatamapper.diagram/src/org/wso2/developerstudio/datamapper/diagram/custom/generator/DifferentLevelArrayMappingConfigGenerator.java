...
public class DifferentLevelArrayMappingConfigGenerator extends AbstractMappingConfigGenerator {
	...
	private void updateOutputVariableForLoopMap(DMVariable outputVariable,
			...
			List<Integer> operationForLoopBeansList) throws DataMapperException {
				...
				if (SchemaDataType.ARRAY.equals(variableType)) {
					if (!outputArrayVariableForLoopMap.containsKey(variableName)) {
						...
						if (targetForLoopIndex < 0) {
							int indexOfForLoopBean = addForLoopInToMap(variableName, new ForLoopBean("i_" + nextName,
									variableName, variableMap.get(variableName).contains(SchemaDataType.NULL)));
							addForLoopBeanIndexToParent(variableName, indexOfForLoopBean,
	...
	private String transformForLoopBeansToJS(ForLoopBean forLoopBean, List<MappingOperation> mappingOperationList,
		...
		List<String> arrayTypeVariableList = getArrayTypeVariablesList(forLoopBean, mappingOperationList, map);

		// Initiating objects of Array type variables"
		functionBuilder.append(initiateArrayTypeVariables(forLoopBean, tempForLoopBeanParentStack, map));
		functionBuilder.append("\n");
		...
		tempForLoopBeanParentStack = (Stack<ForLoopBean>) forLoopBeanParentStack.clone();
		// Initiating arrays of the output variable going to instantiate in
		// below for loop
		functionBuilder.append(initiateArrayTypeVariablesObjects(forLoopBean, tempForLoopBeanParentStack, map));
		functionBuilder.append("\n");

		tempForLoopBeanParentStack = (Stack<ForLoopBean>) forLoopBeanParentStack.clone();
		// Initiating objects of the output variable
		functionBuilder.append(initiateRecordTypeVariables(forLoopBean, tempForLoopBeanParentStack, map));
		functionBuilder.append("\n");
	...
	private Object initiateRecordTypeVariables(ForLoopBean forLoopBean, Stack<ForLoopBean> tempForLoopBeanParentStack,
		...
		for (String variableName : forLoopBean.getObjectVariableListToInstantiate()) {
			...
			variableName = ScriptGenerationUtil.getPrettyVariableNameInForOperation(
					new DMVariable(variableName, "", DMVariableType.INPUT, SchemaDataType.OBJECT, -1), map,
	...
	private String initiateArrayTypeVariablesObjects(ForLoopBean forLoopBean,
			Stack<ForLoopBean> tempForLoopBeanParentStack, Map<String, List<SchemaDataType>> map) {
		StringBuilder functionBuilder = new StringBuilder();
		for (String variableName : forLoopBean.getArrayVariableListToInstantiate()) {
			functionBuilder.append("\n");
			variableName = ScriptGenerationUtil.getPrettyVariableNameInForOperation(
					new DMVariable(variableName, "", DMVariableType.INPUT, SchemaDataType.ARRAY, -1), map,
					tempForLoopBeanParentStack, false);
			functionBuilder.append(variableName + " = {};");
		}
		functionBuilder.append("\n");
				...
				if (DMVariableType.OUTPUT.equals(outputVariable.getType())) {
					...
					if (map.containsKey(variableName)) {
						if (fieldName.contains(SCHEMA_ATTRIBUTE_PREFIX) && ScriptGenerationUtil
								.isVariableTypePrimitive(map.get(variableName).get(VARIABLE_TYPE_INDEX))) {
							String variableString = ScriptGenerationUtil.getPrettyVariableNameInForOperation(
									outputVariable, map, tempForLoopBeanParentStack, true);
