...
public class ConfigureURLrewriteruleDialog extends Dialog {
     ...
     * @param parentShell
     */
    public ConfigureURLrewriteruleDialog(Shell parentShell, URLRewriteMediator urlMediator,
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        for (URLRewriteRule urlRewriteRule : urlRewriteRules) {
            ...
            if (null != ruleCondition) {
                rulesWrapper.setCondition(EsbFactory.eINSTANCE.copyEvaluatorExpressionProperty(ruleCondition));
            }
    ...
    public void unbindAction(TableItem item) {
        ...
        if (action.getUrlRewriteRuleAction().eContainer() != null) {
            RemoveCommand removeCommand = new RemoveCommand(editingDomain, rulesWrapper.getUrlRule(),
    ...
    private void createActionContorls(final TableItem item, final Shell newshell) {
        ...
        final Combo comboAction = createCombo(new String[] { RuleActionType.get(0).getLiteral(),
                RuleActionType.get(1).getLiteral(), RuleActionType.get(2).getLiteral(),
                RuleActionType.get(3).getLiteral(), RuleActionType.get(4).getLiteral() });
        ...
        final Combo comboFragment = createCombo(
                new String[] { RuleFragmentType.get(0).getLiteral(), RuleFragmentType.get(1).getLiteral(),
                        RuleFragmentType.get(2).getLiteral(), RuleFragmentType.get(3).getLiteral(),
                        RuleFragmentType.get(4).getLiteral(), RuleFragmentType.get(5).getLiteral(),
                        RuleFragmentType.get(-1).getLiteral(), RuleFragmentType.get(-2).getLiteral() });
        ...
        final Combo comboOption = createCombo(
                new String[] { RuleOptionType.get(0).getLiteral(), RuleOptionType.get(1).getLiteral() });
        ...
        btnNS.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent e) {
                ...
                NamespacedPropertyEditorDialog dialog = new NamespacedPropertyEditorDialog(newshell,
                        namespacedProperty);
                dialog.open();
    ...
    private void enableControls(final Combo comboOption, final Text text, final Button btnNS, final Text txtreg,
            int actionIndex, int optionIndex) {
    ...
    private void unbindRules(int itemIndex) {
        ...
        if (wraprule.getUrlRule().eContainer() != null) {
            RemoveCommand removeCommand = new RemoveCommand(editingDomain, urlMediator,
                    EsbPackage.Literals.URL_REWRITE_MEDIATOR__URL_REWRITE_RULES, wraprule.getUrlRule());
            getResultCommand().append(removeCommand);
        }
    }
        ...
        for (TableItem tableItem : items) {
            ...
            if (null == rule.eContainer()) {
                ...
                if (rule.getUrlRewriteRuleCondition() == null
                        || !(StringUtils.equals(rule.getUrlRewriteRuleCondition().getEvaluatorValue(),
                                wraprule.getCondition().getEvaluatorValue()))) {

                    SetCommand command = new SetCommand(editingDomain, rule,
                            EsbPackage.Literals.URL_REWRITE_RULE__URL_REWRITE_RULE_CONDITION, wraprule.getCondition());
                ...
                for (UrlActionWrapper urlActionWrapper : actions) {
                    ...
                    if (null == ruleAction.eContainer()) {
                        ...
                        if (urlActionWrapper.getNsproperty() != null) {
                            property = EsbFactory.eINSTANCE.createNamespacedProperty();
                            property.setNamespaces(urlActionWrapper.getNsproperty().getNamespaces());
                            property.setPrettyName("ns");
                            property.setPropertyName("ns");
                            property.setPropertyValue(urlActionWrapper.getNsproperty().getPropertyValue());
                        ...
                        AddCommand addCommand = new AddCommand(editingDomain, rule,
                                EsbPackage.Literals.URL_REWRITE_RULE__REWRITE_RULE_ACTION, ruleAction);
                        getResultCommand().append(addCommand);
                    } else {
                        SetCommand setCommand = new SetCommand(editingDomain, ruleAction,
                        ...
                        setCommand = new SetCommand(editingDomain, ruleAction,
                                EsbPackage.Literals.URL_REWRITE_RULE_ACTION__ACTION_REGEX, urlActionWrapper.getRegex());
                        getResultCommand().append(setCommand);

                        setCommand = new SetCommand(editingDomain, ruleAction,
                                EsbPackage.Literals.URL_REWRITE_RULE_ACTION__ACTION_VALUE, urlActionWrapper.getValue());
                        getResultCommand().append(setCommand);
