...
public class ConfigureEJBMediatorMethodArgumentsDialog extends Dialog {
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        methodArgumentsTable = new Table(container, SWT.BORDER | SWT.FULL_SELECTION | SWT.HIDE_SELECTION);

        FormData methodArgumentsTableLayoutData = new FormData();
        methodArgumentsTableLayoutData.bottom = new FormAttachment(methodArgumentsLabel, 194, SWT.BOTTOM);
        ...
        for (TableItem item : methodArgumentsTable.getItems()) {
            ...
            if (null == argument.eContainer()) {
                ...
                if (item.getText(0).equals(LITERAL_VALUE)) {
                    ...
                    SetCommand setCmdValueType = new SetCommand(editingDomain, argument,
                    ...
                    if (!argument.getPropertyValue().equals(item.getText(2))) {
                        SetCommand setCmd = new SetCommand(editingDomain, argument,
                ...
                if (item.getText(0).equals(LITERAL_EXPRESSION)) {
                    ...
                    if (argument.getPropertyExpression() == null) {
                        ...
                        AddCommand addCmd = new AddCommand(editingDomain, argument,
                                EsbPackage.Literals.ABSTRACT_NAME_VALUE_EXPRESSION_PROPERTY__PROPERTY_EXPRESSION,
                                namespaceProperty);
                        getResultCommand().append(addCmd);
                    } else {
                        SetCommand setCmd = new SetCommand(editingDomain, argument.getPropertyExpression(),
                                EsbPackage.Literals.NAMESPACED_PROPERTY__PROPERTY_VALUE, item.getText(1));
                        getResultCommand().append(setCmd);
    ...
    private TableItem bindMethodArgument(MethodArgument argument) {
        ...
        if (argument.getPropertyValueType().getLiteral().equals("LITERAL")) {
            item.setText(new String[] { LITERAL_VALUE, argument.getPropertyValue() });
        }
        if (argument.getPropertyValueType().getLiteral().equals("EXPRESSION")) {
            item.setText(new String[] { LITERAL_EXPRESSION, argument.getPropertyExpression().getPropertyValue() });
        }

        item.setData(argument);
        item.setData("exp", EsbFactory.eINSTANCE.copyNamespacedProperty(argument.getPropertyExpression()));
        return item;
    }
