...
public class ConfigureAPIHandlersDialog extends Dialog {
    ...
    private void unbindHandler(int itemIndex) {
        ...
        if (null != handler.eContainer()) {
            RemoveCommand removeCmd = new RemoveCommand(editingDomain, api, EsbPackage.Literals.SYNAPSE_API__HANDLERS,
                    handler);
    ...
    private void addHandlerProperty(WrapperProperty wrapperProperty, String propertyName, String propertyValue) {
        int selectedIndex = handlerClassesTale.getSelectionIndex();
        if (selectedIndex != -1) {
        ...
        for (TableItem item : handlerClassesTale.getItems()) {
            ...
            if (null == handler.eContainer()) {
                if (wrapperHandlerClassName != null && !wrapperHandlerClassName.equals("")) {
                    ...
                    AddCommand addCmd = new AddCommand(editingDomain, api, EsbPackage.Literals.SYNAPSE_API__HANDLERS,
                            handler);
