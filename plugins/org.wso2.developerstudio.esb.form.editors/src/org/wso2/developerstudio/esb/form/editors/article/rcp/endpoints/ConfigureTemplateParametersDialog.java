...
public class ConfigureTemplateParametersDialog extends Dialog {
	...
	private List<TemplateParameter> templateParameterList = new ArrayList<TemplateParameter>();
	...
	public ConfigureTemplateParametersDialog(Shell parentShell, List<TemplateParameter> list) {
				super(parentShell);
				// When updating an existing parameter, then get the parameter list
				if (list != null) {
	...
	protected Control createDialogArea(Composite parent) {
				...
				| SWT.FULL_SELECTION | SWT.HIDE_SELECTION);
		{
			...
			// Populate template.
			for (TemplateParameter parameter : templateParameterList) {
	...
	private void unbindParameter(int itemIndex) {
		...
		removeTaskProperty(Parameter);
		parametersTable.remove(parametersTable.indexOf(item));
	}
	
	private void removeTaskProperty(TemplateParameter param) {
		if (param != null) {
			for (TemplateParameter propertyItem : getTemplateParameterList()) {
				if (propertyItem.getName().equals(param.getName())) {
					getTemplateParameterList().remove(propertyItem);
					break;
				}
			}
		}
	...
	protected void okPressed() {
		...
		for (TableItem item : parametersTable.getItems()) {
			...
			if (item.getData() == null) {
				parameter = EsbFactory.eINSTANCE.createTemplateParameter();;
				parameter.setName(item.getText(0));					
			} else {
				parameter = (TemplateParameter) item.getData();
				parameter.setName(item.getText(0));
			}
			...
			getTemplateParameterList().add(parameter);
		}
		super.okPressed();
	}
