...
public class ThrottleMediatorDeserializer extends AbstractEsbNodeDeserializer<AbstractMediator, ThrottleMediator> {
    ...
    public ThrottleMediator createNode(IGraphicalEditPart part, AbstractMediator mediator)
        ...
        Assert.isTrue(mediator instanceof org.apache.synapse.mediators.throttle.ThrottleMediator,
        ...
        ThrottleMediator visualThrottle = (ThrottleMediator) DeserializerUtils.createNode(part,
        ...
        if (throttleMediator.getPolicyKey() != null) {
            ...
            if (throttlePolicy != null) {
                ...
                if (mediatorThrottleAssertion != null) {
                    ...
                    Iterator<OMElement> maximumConcAccIterator = mediatorThrottleAssertion
                            .getChildrenWithLocalName("MaximumConcurrentAccess");
                    ...
                    Iterator<OMElement> it = mediatorThrottleAssertion.getChildrenWithLocalName("Policy"); it
                            .hasNext();) {
        ...
        if (throttleMediator.getOnAcceptMediator() != null) {
            if (throttleMediator.getOnAcceptMediator() instanceof SequenceMediator) {
                MediatorFlow mediatorFlow = visualThrottle.getThrottleContainer().getOnAcceptContainer()
                        .getMediatorFlow();
                GraphicalEditPart compartment = (GraphicalEditPart) ((getEditpart(mediatorFlow)).getChildren().get(0));
                deserializeSequence(compartment, (SequenceMediator) throttleMediator.getOnAcceptMediator(),
                        visualThrottle.getOnAcceptOutputConnector());
            } else {
                getLog().warn(
                        "Ignoring configuration : throttle OnAcceptMediator is not an instanceof SequenceMediator");
            }
            ...
            if (StringUtils.isNotBlank(onAcceptSeqKey)) {
                executeSetValueCommand(THROTTLE_MEDIATOR__ON_ACCEPT_BRANCHSEQUENCE_TYPE,
                        ThrottleSequenceType.REGISTRY_REFERENCE);
                RegistryKeyProperty keyProperty = EsbFactory.eINSTANCE.createRegistryKeyProperty();
        ...
        if (throttleMediator.getOnRejectMediator() != null) {
            if (throttleMediator.getOnRejectMediator() instanceof SequenceMediator) {
                MediatorFlow mediatorFlow = visualThrottle.getThrottleContainer().getOnRejectContainer()
                        .getMediatorFlow();
                GraphicalEditPart compartment = (GraphicalEditPart) ((getEditpart(mediatorFlow)).getChildren().get(0));
                deserializeSequence(compartment, (SequenceMediator) throttleMediator.getOnRejectMediator(),
                        visualThrottle.getOnRejectOutputConnector());
            } else {
                getLog().warn(
                        "Ignoring configuration : throttle OnRejectMediator is not an instanceof SequenceMediator");
            }
            ...
            if (StringUtils.isNotBlank(onRejectSeqKey)) {
                executeSetValueCommand(THROTTLE_MEDIATOR__ON_REJECT_BRANCHSEQUENCE_TYPE,
                        ThrottleSequenceType.REGISTRY_REFERENCE);
                RegistryKeyProperty keyProperty = EsbFactory.eINSTANCE.createRegistryKeyProperty();
    ...
    private ThrottlePolicyEntry createVisualPolicyEntry(OMElement outerPolicy) {
        ...
        if (policyIdElem != null) {
            ...
            // get type attribute
            OMAttribute typeAttribute = policyIdElem
                    .getAttribute(new QName("http://www.wso2.org/products/wso2commons/throttle", "type"));
    ...
    private void configureAccessControlParams(ThrottlePolicyEntry visualPolicyEntry, OMElement accessTypeElem) {
        ...
        if (maximumCountElem != null && maximumCountElem.getText() != null
                && DeserializerUtils.isInteger(maximumCountElem.getText().trim())) {
        ...
        if (unitTimeElem != null && unitTimeElem.getText() != null
                && DeserializerUtils.isInteger(unitTimeElem.getText())) {
        ...
        if (prohibitTimePeriodElem != null && prohibitTimePeriodElem.getText() != null
                && DeserializerUtils.isInteger(prohibitTimePeriodElem.getText())) {
