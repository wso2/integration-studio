...
public class EvenlyDividedFixedBorderItemLocator extends BorderItemLocator {
     ...
     */
    public EvenlyDividedFixedBorderItemLocator(IFigure parentFigure, IFigure borderItemFigure, int side, int id,
            int siblingCount) {
    ...
    protected Point locateOnBorder(Point suggestedLocation, int suggestedSide, int circuitCount, IFigure borderItem) {
        ...
        if (suggestedSide == PositionConstants.WEST) {
            resultX = parentFigureX - borderItemSize.width + getBorderItemOffset().width;
            resultY = parentFigureY + (parentFigureHeight * unitSpace * id) - (borderItemSize.height / 2);
        } else if (suggestedSide == PositionConstants.SOUTH) {
            resultX = parentFigureX + (parentFigureWidth * unitSpace * id) - (borderItemSize.width / 2);
            resultY = parentFigureY - borderItemSize.height + getBorderItemOffset().height;
        } else if (suggestedSide == PositionConstants.EAST) {
            resultX = parentFigureX + parentFigureWidth - borderItemSize.width + getBorderItemOffset().width;
            resultY = parentFigureY + (parentFigureHeight * unitSpace * id) - (borderItemSize.height / 2);
        } else {
            resultX = parentFigureX + (parentFigureWidth * unitSpace * id) - (borderItemSize.width / 2);
            resultY = parentFigureY - borderItemSize.height + getBorderItemOffset().height;
