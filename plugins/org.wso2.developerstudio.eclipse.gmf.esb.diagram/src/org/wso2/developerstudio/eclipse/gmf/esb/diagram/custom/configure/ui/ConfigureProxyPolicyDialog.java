    ...
    protected Control createDialogArea(Composite parent) {
        ...
        newPolicyButton.addListener(SWT.Selection, new Listener() {
            public void handleEvent(Event event) {
                TableItem item = bindPram(EsbFactory.eINSTANCE.createProxyServicePolicy());
        ...
        removePolicyButtonLayoutData.top = new FormAttachment(newPolicyButton, 5);
        removePolicyButtonLayoutData.right = new FormAttachment(100);
        removePolicyButtonLayoutData.left = new FormAttachment(newPolicyButton, 0, SWT.LEFT);
        ...
        policyTable = new Table(container, SWT.BORDER | SWT.FULL_SELECTION | SWT.HIDE_SELECTION);
        ...
        proxyPolicyTableLayoutData.top = new FormAttachment(newPolicyButton, 0, SWT.TOP);
        proxyPolicyTableLayoutData.left = new FormAttachment(0);
        proxyPolicyTableLayoutData.right = new FormAttachment(newPolicyButton, -5);
        ...
        for (TableItem item : policyTable.getItems()) {
            ...
            if (policies.eContainer() == null) {
                AddCommand addCmd = new AddCommand(editingDomain, proxyService,
                        EsbPackage.Literals.PROXY_SERVICE__SERVICE_POLICIES, policies);
    ...
    private void unbindParam(int itemIndex) {
        ...
        if (policy.eContainer() != null) {
            RemoveCommand removeCmd = new RemoveCommand(editingDomain, proxyService,
                    EsbPackage.Literals.PROXY_SERVICE__SERVICE_POLICIES, policy);
