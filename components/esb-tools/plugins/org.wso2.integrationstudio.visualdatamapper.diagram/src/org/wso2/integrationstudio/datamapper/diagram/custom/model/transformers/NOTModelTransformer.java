...
import org.wso2.integrationstudio.datamapper.impl.NOTImpl;
import org.wso2.integrationstudio.datamapper.impl.OperatorImpl;

public class NOTModelTransformer implements IModelTransformer {

	@Override
	public DMOperation transform(OperatorImpl operator, int index) {
		if (operator instanceof NOTImpl) {
			DMOperation notOperatorModel = new DMOperation(DataMapperOperatorType.NOT, operator.toString(), index);
			return notOperatorModel;
		}
		throw new IllegalArgumentException("NOTImpl operator expected. Found : " + operator.toString());
	}
