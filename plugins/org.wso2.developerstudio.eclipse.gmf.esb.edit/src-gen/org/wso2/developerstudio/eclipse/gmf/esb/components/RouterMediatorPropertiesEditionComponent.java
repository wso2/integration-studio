...
public class RouterMediatorPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
     ...
     * 
     */
    public RouterMediatorPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject routerMediator,
            String editing_mode) {
    ...
    public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
        ...
        if (editingPart != null && key == partKey) {
            ...
            if (isAccessible(EsbViewsRepository.RouterMediator.Properties.targetOutputConnector)) {
                targetOutputConnectorSettings = new ReferencesTableSettings(routerMediator,
                ...
                basePart.addFilterToTargetOutputConnector(new ViewerFilter() {
                    ...
                    public boolean select(Viewer viewer, Object parentElement, Object element) {
                        return (element instanceof String && element.equals("")) //$NON-NLS-1$
                                || (element instanceof RouterMediatorTargetOutputConnector);
                    }
    ...
    public void updateSemanticModel(final IPropertiesEditionEvent event) {
        ...
        if (EsbViewsRepository.RouterMediator.Properties.description == event.getAffectedEditor()) {
            routerMediator.setDescription((java.lang.String) EEFConverterUtil
        ...
        if (EsbViewsRepository.RouterMediator.Properties.targetOutputConnector == event.getAffectedEditor()) {
            if (event.getKind() == PropertiesEditionEvent.ADD) {
                EReferencePropertiesEditionContext context = new EReferencePropertiesEditionContext(editingContext,
                        this, targetOutputConnectorSettings, editingContext.getAdapterFactory());
                ...
                targetOutputConnectorSettings.move(event.getNewIndex(),
                        (RouterMediatorTargetOutputConnector) event.getNewValue());
            }
        }
    }
    ...
    public void updatePart(Notification msg) {
        ...
        if (editingPart.isVisible()) {
            ...
            if (EsbPackage.eINSTANCE.getEsbElement_Description().equals(msg.getFeature())
                    ...
                    && isAccessible(EsbViewsRepository.RouterMediator.Properties.description)) {
            ...
            if (EsbPackage.eINSTANCE.getEsbElement_CommentsList().equals(msg.getFeature())
                    && msg.getNotifier().equals(semanticObject) && basePart != null
                    && isAccessible(EsbViewsRepository.RouterMediator.Properties.commentsList)) {
            ...
            if (EsbPackage.eINSTANCE.getMediator_Reverse().equals(msg.getFeature())
                    && msg.getNotifier().equals(semanticObject) && basePart != null
                    && isAccessible(EsbViewsRepository.RouterMediator.Properties.reverse))
                basePart.setReverse((Boolean) msg.getNewValue());

            if (EsbPackage.eINSTANCE.getRouterMediator_ContinueAfterRouting().equals(msg.getFeature())
                    && msg.getNotifier().equals(semanticObject) && basePart != null
                    && isAccessible(EsbViewsRepository.RouterMediator.Properties.continueAfterRouting))
                basePart.setContinueAfterRouting((Boolean) msg.getNewValue());

            if (EsbPackage.eINSTANCE.getRouterMediator_TargetOutputConnector().equals(msg.getFeature())
                    && isAccessible(EsbViewsRepository.RouterMediator.Properties.targetOutputConnector))
        ...
        if (event.getNewValue() != null) {
            try {
                ...
                if (EsbViewsRepository.RouterMediator.Properties.continueAfterRouting == event.getAffectedEditor()) {
                    ...
                    if (newValue instanceof String) {
                        newValue = EEFConverterUtil.createFromString(
                                EsbPackage.eINSTANCE.getRouterMediator_ContinueAfterRouting().getEAttributeType(),
                                (String) newValue);
                    }
