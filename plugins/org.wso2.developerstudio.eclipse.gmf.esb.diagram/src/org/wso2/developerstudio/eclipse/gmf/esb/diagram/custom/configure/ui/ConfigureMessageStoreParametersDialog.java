...
public class ConfigureMessageStoreParametersDialog extends Dialog {
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        parameterLabel = new Label(container, SWT.NONE);
        {
            ...
            proxyParameterLabelLayoutData.top = new FormAttachment(dummy, 10);
        ...
        newParameterButton = new Button(container, SWT.NONE);
        {
            ...
            newProxyParameterButtonLayoutData.top = new FormAttachment(parameterLabel, 10);
            ...
            newParameterButton.addListener(SWT.Selection, new Listener() {
                public void handleEvent(Event event) {
                    TableItem item = bindParameter(EsbFactory.eINSTANCE.createMessageStoreParameter());
        ...
        removeParameterButton = new Button(container, SWT.NONE);
        {
            ...
            removeProxyParameterButtonLayoutData.top = new FormAttachment(newParameterButton, 5);
            removeProxyParameterButtonLayoutData.right = new FormAttachment(100);
            removeProxyParameterButtonLayoutData.left = new FormAttachment(newParameterButton, 0, SWT.LEFT);
            removeParameterButton.setLayoutData(removeProxyParameterButtonLayoutData);
    ...
    private void unbindParameter(int itemIndex) {
        ...
        if (null != Parameter.eContainer()) {
            RemoveCommand removeCmd = new RemoveCommand(editingDomain, store,
    ...
    protected void okPressed() {
        ...
        for (TableItem item : parametersTable.getItems()) {
            ...
            if (null == parameter.eContainer()) {
                ...
                AddCommand addCmd = new AddCommand(editingDomain, store, EsbPackage.Literals.MESSAGE_STORE__PARAMETERS,
                        parameter);
                ...
                if (!parameter.getParameterName().equals(item.getText(0))) {
                    SetCommand setCmd = new SetCommand(editingDomain, parameter,
                            EsbPackage.Literals.MESSAGE_STORE_PARAMETER__PARAMETER_NAME, item.getText(0));
                    getResultCommand().append(setCmd);
                }

                if (!parameter.getParameterValue().equals(item.getText(1))) {
                    SetCommand setCmd = new SetCommand(editingDomain, parameter,
                            EsbPackage.Literals.MESSAGE_STORE_PARAMETER__PARAMETER_VALUE, item.getText(1));
                    getResultCommand().append(setCmd);
                }
            }
        }
