...
import org.wso2.integrationstudio.eclipse.usermgt.remote.UserManagerUtils;
...
public abstract class UserRolesDialog extends Dialog {
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        userRolesTable = new Table(container, SWT.BORDER | SWT.FULL_SELECTION | SWT.HIDE_SELECTION | SWT.CHECK);
        {
    ...
    private void listRoles() {
        ...
        if (StringUtils.isNotBlank(serverUrlText.getText()) && StringUtils.isNotBlank(userNameText.getText())
                && StringUtils.isNotBlank(passwordText.getText())) {
            UserManagerUtils.getInstance().init(serverUrlText.getText().trim(), userNameText.getText().trim(),
