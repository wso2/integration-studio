...
public class ANDModelTransformer implements IModelTransformer {

	@Override
	public DMOperation transform(OperatorImpl operator, int index) {
		if (operator instanceof ANDImpl) {
			DMOperation andOperatorModel = new DMOperation(DataMapperOperatorType.AND, operator.toString(), index);
			return andOperatorModel;
		}
		throw new IllegalArgumentException("ANDImpl operator expected. Found : " + operator.toString());
	}
