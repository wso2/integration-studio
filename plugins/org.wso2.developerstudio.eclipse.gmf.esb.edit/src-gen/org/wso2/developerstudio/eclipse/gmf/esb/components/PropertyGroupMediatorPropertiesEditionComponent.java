...
public class PropertyGroupMediatorPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public PropertyGroupMediatorPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject propertyGroupMediator, String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(EsbViewsRepository.PropertyGroupMediator.Properties.description))
				basePart.setDescription(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, propertyGroupMediator.getDescription()));
			...
			if (isAccessible(EsbViewsRepository.PropertyGroupMediator.Properties.properties_)) {
				propertiesSettings = new ReferencesTableSettings(propertyGroupMediator, EsbPackage.eINSTANCE.getPropertyGroupMediator_Properties());
			...
			if (isAccessible(EsbViewsRepository.PropertyGroupMediator.Properties.properties_)) {
				basePart.addFilterToProperties(new ViewerFilter() {
					...
					public boolean select(Viewer viewer, Object parentElement, Object element) {
						return (element instanceof String && element.equals("")) || (element instanceof PropertyMediator); //$NON-NLS-1$ 
					}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (EsbViewsRepository.PropertyGroupMediator.Properties.description == event.getAffectedEditor()) {
			propertyGroupMediator.setDescription((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (EsbPackage.eINSTANCE.getEsbElement_Description().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(EsbViewsRepository.PropertyGroupMediator.Properties.description)) {
			...
			if (EsbPackage.eINSTANCE.getEsbElement_CommentsList().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(EsbViewsRepository.PropertyGroupMediator.Properties.commentsList)) {
			...
			if (EsbPackage.eINSTANCE.getMediator_Reverse().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(EsbViewsRepository.PropertyGroupMediator.Properties.reverse))
				basePart.setReverse((Boolean)msg.getNewValue());
			
			if (EsbPackage.eINSTANCE.getPropertyGroupMediator_Properties().equals(msg.getFeature()) && isAccessible(EsbViewsRepository.PropertyGroupMediator.Properties.properties_))
