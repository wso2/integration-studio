...
import org.wso2.integrationstudio.eclipse.gmf.esb.diagram.custom.RegistryKeyPropertyUtil;
...
public class ConfigureRouterMediatorDialog extends Dialog {
     ...
     * @param parentShell
     */
    public ConfigureRouterMediatorDialog(Shell parentShell, RouterMediator routerMediator,
    ...
    private TableItem bindRoute(RouteWrapper wrapper) {
        ...
        item.setText("Route");
        item.setImage(SWTResourceManager.getImage(this.getClass(), "/icons/custom/12px/router.png"));
        ...
        wrapper.setSequenceKey(EsbFactory.eINSTANCE.copyRegistryKeyProperty(
                container.getTarget().getSequenceKey())); /* we read this even for anonymous sequence */
    ...
    private void unbindRoute(int itemIndex) {
        ...
        if (null != target.eContainer()) {
            ...
            RemoveCommand removeCmd = new RemoveCommand(editingDomain, routerMediator.getRouterContainer(),
                    EsbPackage.Literals.ROUTER_MEDIATOR_CONTAINER__ROUTER_TARGET_CONTAINER, target);
    ...
    protected void okPressed() {
        for (TableItem item : tblRoutes.getItems()) {
            ...
            if (null == target.eContainer()) {
                AddCommand addCmd = new AddCommand(editingDomain, routerMediator.getRouterContainer(),
                        EsbPackage.Literals.ROUTER_MEDIATOR_CONTAINER__ROUTER_TARGET_CONTAINER, target);
            ...
            if (!target.getRouteExpression().equals(wrapper.getRouteExpression())) {
                setCmd = new SetCommand(editingDomain, target,
                        EsbPackage.Literals.ROUTER_TARGET_CONTAINER__ROUTE_EXPRESSION, wrapper.getRouteExpression());
                getResultCommand().append(setCmd);
            }

            if (target.isBreakAfterRoute() != wrapper.isBreakAfterRoute()) {
                setCmd = new SetCommand(editingDomain, target,
                        EsbPackage.Literals.ROUTER_TARGET_CONTAINER__BREAK_AFTER_ROUTE, wrapper.isBreakAfterRoute());
                getResultCommand().append(setCmd);
            }

            if (!target.getRoutePattern().equals(wrapper.getRoutePattern())) {
                setCmd = new SetCommand(editingDomain, target,
                        EsbPackage.Literals.ROUTER_TARGET_CONTAINER__ROUTE_PATTERN, wrapper.getRoutePattern());
                getResultCommand().append(setCmd);
            }

            if ((target.getTarget().getSequenceType() == TargetSequenceType.REGISTRY_REFERENCE) != wrapper
                    .isDynamicSequence()) {
                setCmd = new SetCommand(editingDomain, target.getTarget(),
                        EsbPackage.Literals.ABSTRACT_COMMON_TARGET__SEQUENCE_TYPE,
                        wrapper.isDynamicSequence() ? TargetSequenceType.REGISTRY_REFERENCE
            ...
            if (wrapper.isDynamicSequence()) {
                setCmd = new SetCommand(editingDomain, target.getTarget(),
                        EsbPackage.Literals.ABSTRACT_COMMON_TARGET__SEQUENCE_KEY, wrapper.getSequenceKey());
                getResultCommand().append(setCmd);
            } else {
                setCmd = new SetCommand(editingDomain, target.getTarget(),
