...
import org.wso2.integrationstudio.datamapper.impl.StringToNumberImpl;

public class StringToNumberModelTransformer implements IModelTransformer {

	@Override
	public DMOperation transform(OperatorImpl operator, int index) {
		if (operator instanceof StringToNumberImpl) {
			DMOperation stringToNumberOperatorModel = new DMOperation(DataMapperOperatorType.STRING_TO_NUMBER,
					operator.toString(), index);
			return stringToNumberOperatorModel;
		}
		throw new IllegalArgumentException("StringToNumberImpl operator expected. Found : " + operator.toString());
	}
}
