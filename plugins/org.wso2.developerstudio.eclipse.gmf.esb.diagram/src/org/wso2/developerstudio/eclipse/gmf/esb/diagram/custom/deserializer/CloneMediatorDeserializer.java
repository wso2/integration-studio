...
public class CloneMediatorDeserializer extends AbstractEsbNodeDeserializer<AbstractMediator, CloneMediator> {
    ...
    public CloneMediator createNode(IGraphicalEditPart part, AbstractMediator mediator) throws DeserializerException {
        ...
        Assert.isTrue(mediator instanceof org.apache.synapse.mediators.eip.splitter.CloneMediator,
        ...
        CloneMediator visualClone = (CloneMediator) DeserializerUtils.createNode(part,
        ...
        for (Target target : cloneMediator.getTargets()) {
            ...
            if (count++ == 0 && visualClone.getTargets().size() == 1) {
                ...
                targetOutputConnector = (CloneMediatorTargetOutputConnector) DeserializerUtils.createNode(
                        (IGraphicalEditPart) getEditpart(visualClone),
            ...
            if (target.getSequence() != null) {
                executeSetValueCommand(visualTarget, ABSTRACT_COMMON_TARGET__SEQUENCE_TYPE,
                        TargetSequenceType.ANONYMOUS);
                refreshEditPartMap();
                IGraphicalEditPart compartment = (IGraphicalEditPart) getEditpart(
                        cloneTargetContainer.getMediatorFlow()).getChildren().get(0);
            ...
            if (target.getEndpoint() != null) {
                executeSetValueCommand(visualTarget, ABSTRACT_COMMON_TARGET__ENDPOINT_TYPE,
                ...
                IEsbNodeDeserializer deserializer = EsbDeserializerRegistry.getInstance()
                        .getDeserializer(target.getEndpoint());

                if (target.getEndpoint() instanceof FailoverEndpoint) {
                    @SuppressWarnings("unchecked")
                    EndPoint visualEndPoint = (EndPoint) deserializer
                            .createNode((IGraphicalEditPart) getEditpart(cloneTargetContainer.getMediatorFlow())
                                    .getChildren().get(0), target.getEndpoint());
                    ...
                    @SuppressWarnings("unchecked")
                    AbstractEndPoint visualEndPoint = (AbstractEndPoint) deserializer
                            .createNode((IGraphicalEditPart) getEditpart(cloneTargetContainer.getMediatorFlow())
                                    .getChildren().get(0), target.getEndpoint());
