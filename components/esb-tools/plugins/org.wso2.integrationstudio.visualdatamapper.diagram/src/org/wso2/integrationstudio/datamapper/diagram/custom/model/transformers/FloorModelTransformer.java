...
import org.wso2.integrationstudio.datamapper.impl.FloorImpl;
import org.wso2.integrationstudio.datamapper.impl.OperatorImpl;

public class FloorModelTransformer implements IModelTransformer {

	@Override
	public DMOperation transform(OperatorImpl operator, int index) {
		if (operator instanceof FloorImpl) {
			DMOperation floorOperatorModel = new DMOperation(DataMapperOperatorType.FLOOR, operator.toString(), index);
			return floorOperatorModel;
		}
		throw new IllegalArgumentException("FloorImpl operator expected. Found : " + operator.toString());
	}
