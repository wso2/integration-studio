...
public class ConfigurePayloadFactoryMediatorDialog extends Dialog {
     ...
     * @param editingDomain
     */
    public ConfigurePayloadFactoryMediatorDialog(Shell parentShell, PayloadFactoryMediator mediator,
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        cmdAddArgument.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                TableItem item = bindArgument(EsbFactory.eINSTANCE.createPayloadFactoryArgument());
    ...
    private void unbindArgument(int itemIndex) {
        ...
        if (null != argument.eContainer()) {
            RemoveCommand removeCmd = new RemoveCommand(editingDomain, mediator,
                    EsbPackage.Literals.PAYLOAD_FACTORY_MEDIATOR__ARGS, argument);
    ...
    protected void okPressed() {
        for (TableItem item : tblArguments.getItems()) {
            ...
            if ((argument.getArgumentType() == PayloadFactoryArgumentType.EXPRESSION) != wrapper.isExpression()) {
                setCmd = new SetCommand(editingDomain, argument,
                        EsbPackage.Literals.PAYLOAD_FACTORY_ARGUMENT__ARGUMENT_TYPE,
                        wrapper.isExpression() ? PayloadFactoryArgumentType.EXPRESSION
            ...
            if (wrapper.isExpression()) {
                setCmd = new SetCommand(editingDomain, argument,
                        EsbPackage.Literals.PAYLOAD_FACTORY_ARGUMENT__EVALUATOR, wrapper.getEvaluator());
                getResultCommand().append(setCmd);
            }
            ...
            if (!argument.getArgumentValue().equals(wrapper.getArgumentValue())) {
                setCmd = new SetCommand(editingDomain, argument,
                        EsbPackage.Literals.PAYLOAD_FACTORY_ARGUMENT__ARGUMENT_VALUE, wrapper.getArgumentValue());
                getResultCommand().append(setCmd);
            }
