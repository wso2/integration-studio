...
import org.wso2.integrationstudio.eclipse.gmf.esb.diagram.custom.AbstractEndpointAdditionalOutputConnectorEditPart;
...
public class AddbranchEndpointDialog extends Dialog {
    ...
    public AddbranchEndpointDialog(Shell parentShell, EndPoint endpoint, TransactionalEditingDomain editingDomain,
            EditPart editpart) {
        super(parentShell);
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        count = new Text(container, SWT.NONE);
        {
            ...
            countLayoutData.top = new FormAttachment(branchesCount, 0, SWT.CENTER);
            countLayoutData.left = new FormAttachment(branchesCount, 5);
            count.setLayoutData(countLayoutData);
            int i = (endpoint instanceof LoadBalanceEndPoint)
                    ? ((LoadBalanceEndPoint) endpoint).getOutputConnector().size()
                    : ((FailoverEndPoint) endpoint).getOutputConnector().size();
        ...
        if (endpoint instanceof LoadBalanceEndPoint) {
            ...
            if (number > 0) {
                for (int i = 0; i < number; ++i) {
                    LoadBalanceEndPointOutputConnector cb = EsbFactory.eINSTANCE
                            .createLoadBalanceEndPointOutputConnector();
                    AddCommand addCmd = new AddCommand(domain, parentEndpoint,
                            EsbPackage.Literals.LOAD_BALANCE_END_POINT__OUTPUT_CONNECTOR, cb);
                ...
                for (int i = 0; i < Math.abs(number); ++i) {
                    Object lastConnector = parentEndpoint.getOutputConnector()
                            .get(parentEndpoint.getOutputConnector().size() - 1);
            ...
            if (number > 0) {
                for (int i = 0; i < number; ++i) {
                    ...
                    AddCommand addCmd = new AddCommand(domain, parentEndpoint,
                            EsbPackage.Literals.FAILOVER_END_POINT__OUTPUT_CONNECTOR, cb);
                    if (addCmd.canExecute()) {
                        domain.getCommandStack().execute(addCmd);
                    }
                }
    ...
    private void reorder(float number) {
        ...
        for (int i = 0; i < editpart.getChildren().size(); ++i) {
            if (editpart.getChildren().get(i) instanceof AbstractEndpointAdditionalOutputConnectorEditPart) {
                pos = pos + (1 / (number + 1));
                IFigure borderItemFigure = ((AbstractEndpointAdditionalOutputConnectorEditPart) editpart.getChildren()
                        .get(i)).getFigure();
                outputConnectors.add(borderItemFigure);
                BorderItemLocator locator = new FixedBorderItemLocator(((AbstractEndpoint) editpart).getMainFigure(),
                        borderItemFigure, PositionConstants.EAST, pos);
                outputLocators.add(locator);
            }
        }
        for (int i = 0; i < outputConnectors.size(); ++i) {
            ((AbstractEndpoint) editpart).getBorderedFigure().getBorderItemContainer().remove(outputConnectors.get(i));
            ((AbstractEndpoint) editpart).getBorderedFigure().getBorderItemContainer().add(outputConnectors.get(i),
                    outputLocators.get(i));
        }
    }
