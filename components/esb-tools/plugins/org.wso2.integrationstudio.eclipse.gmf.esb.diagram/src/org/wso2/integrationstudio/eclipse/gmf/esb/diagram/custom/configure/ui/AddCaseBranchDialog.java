...
public class AddCaseBranchDialog extends Dialog {
    ...
    protected void okPressed() {
        int number = Integer.parseInt(count.getText())
                - switchMediator.getSwitchContainer().getSwitchCaseParentContainer().getSwitchCaseContainer().size();
        if (number > 0) {
            for (int i = 0; i < number; ++i) {
                ...
                AddCommand addCmd = new AddCommand(editingDomain,
                        switchMediator.getSwitchContainer().getSwitchCaseParentContainer(),
                ...
                AddCommand addCaseOutputConnectorCommand = new AddCommand(editingDomain, switchMediator,
            ...
            for (int i = 0; i < Math.abs(number); ++i) {
                SwitchCaseContainer lastCaseContainer = switchMediator.getSwitchContainer()
                        .getSwitchCaseParentContainer().getSwitchCaseContainer().get(switchMediator.getSwitchContainer()
                                .getSwitchCaseParentContainer().getSwitchCaseContainer().size() - 1);
                ...
                SwitchCaseBranchOutputConnector lastCaseOutputConnector = switchMediator.getCaseBranches()
                        .get(switchMediator.getCaseBranches().size() - 1);
