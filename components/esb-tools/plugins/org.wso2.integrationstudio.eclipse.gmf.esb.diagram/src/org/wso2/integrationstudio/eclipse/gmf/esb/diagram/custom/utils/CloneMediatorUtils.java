...
public class CloneMediatorUtils {
    ...
    private static void reorder(EditPart editpart, int position) {
        ...
        for (int i = 0; i < ((EditPart) editpart.getChildren().get(4)).getChildren().size(); ++i) {
            if (((EditPart) editpart.getChildren().get(4)).getChildren()
                    .get(i) instanceof CloneTargetContainerEditPart) {
        ...
        for (int i = 0; i < outputConnectors.size(); ++i) {
            ...
            ((CloneMediatorEditPart) editpart).getBorderedFigure().getBorderItemContainer().add(outputConnectors.get(i),
    ...
    public static void addTargetInitially(EditPart child, TransactionalEditingDomain domain) {
        ...
        EObject parentContainer = ((org.eclipse.gmf.runtime.notation.impl.NodeImpl) (cloneMediatorEditPart).getModel())
                .getElement();
        if (((CloneMediator) parentContainer).getTargetsOutputConnector().size() == 0) {
            ...
            AddCommand addCmd = new AddCommand(domain, ((CloneMediator) parentContainer).getCloneContainer(),
