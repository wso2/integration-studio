...
package org.wso2.developerstudio.eclipse.artifact.cep.project.refactoring.rename;
...
public class CEPArtifactRenameParticipant extends RenameParticipant {
	...
	public RefactoringStatus checkConditions(IProgressMonitor arg0, CheckConditionsContext arg1)
		...
		if (originalFile != null) {
			...
			FileUtils.getAllExactMatchingFiles(cepProject.getLocation().toOSString(),
			...
			if (!matchinFilesList.isEmpty()) {
				return RefactoringStatus.createFatalErrorStatus("An CEP Artifact already exist with the same name " +
				                                                changedFileName +
				                                                " in the project " +
				                                                cepProject.getName());
			} else if (changedFileName.substring(0, changedFileName.lastIndexOf("."))
			                          .equalsIgnoreCase(cepProject.getName())) {
				return RefactoringStatus.createFatalErrorStatus("You are trying to rename your CEP Artifact to have the project name.");
			}

			return RefactoringStatus.createInfoStatus("You are about the rename your CEP Artifact " +
	...
	public Change createChange(IProgressMonitor arg0) throws CoreException,
		...
		CompositeChange change=new CompositeChange("CEP Artifact Model Rename");
		String originalFileNamewithExtension = originalFile.getName();
		IFile artifactFile = cepProject.getFile("artifact.xml");
		...
		change.add(new CEPMetaDataFileChange("Meta data file", artifactFile, originalNameWithoutExtension, changedNameWithoutextension));
		return change;
	}

	
	public String getName() {
		return "CEPArtifactRename";
	}
